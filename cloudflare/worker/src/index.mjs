...
export default {
  async fetch(request, env) {
    ...
    // 27.08.2026 eklendi — manuel satış akışı: developer, kendi ürettiği
    // (uygulama içi form akışından GEÇMEMİŞ) bir siteyi doğrudan bir
    // müşterinin Sitora hesabına "hazır proje" olarak enjekte edebilsin
    // diye. Müşteri kendi hesabından Projelerim'i açtığında bu proje
    // hazır görünür, kendi eliyle "Yayınla" / "Domain Bağla" yapabilir —
    // ama içeriği developer'ın kendi AI/şablon sistemiyle üretilmiştir,
    // müşteri hiçbir formu görmez/doldurmaz (bkz. handleAdminProjectCreate).
    if (pathname.match(/^\/admin\/users\/[^/]+\/projects$/) && request.method === 'POST') {
      const uid = pathname.split('/')[3];
      return requireAdmin(request, env, async () => {
        const body = await request.json().catch(() => ({}));
        return handleAdminProjectCreate(uid, body, env);
      });
    }
...
/// 27.08.2026 genişletildi: mapValue (iç içe obje) ve arrayValue artık
/// rekürsif olarak düz JS değerine çözülüyor — handleAdminUserProjects
/// artık files/formData gibi iç içe alanları da okuyabilsin diye. Eskiden
/// bu tipler `else out[k] = v` dalına düşüp ham Firestore sarmalayıcısını
/// döndürüyordu; mevcut basit alanlar (string/number/bool) davranışı
/// AYNEN korunuyor, sadece yeni dallar eklendi.
function fsFieldsToPlain(fields) {
  const out = {};
  for (const [k, v] of Object.entries(fields || {})) {
    out[k] = fsValueToPlain(v);
  }
  return out;
}
...
function fsValueToPlain(v) {
  ...
  if ('mapValue' in v) return fsFieldsToPlain(v.mapValue.fields || {});
  if ('arrayValue' in v) return (v.arrayValue.values || []).map(fsValueToPlain);
  return v;
}

/// Ters yön: admin panelinden gelen düz bir { formCredits: 20 } gibi objeyi
/// Firestore PATCH'in beklediği tipli formata çevirir.
/// 27.08.2026 genişletildi: artık iç içe obje (map) alanları da destekler
/// (handleAdminProjectCreate'in files/formData gibi Map<String,*> alanları
/// yazabilmesi için) — mevcut kullanımlar (mail, kullanıcı puanı, rozet
/// bayrağı) hep düz (number/boolean/string) kaldığı için bu değişiklik
/// onları ETKİLEMEZ, sadece yeni bir dal (object -> mapValue) ekler.
function plainToFsFields(obj) {
  const fields = {};
  for (const [k, v] of Object.entries(obj)) {
    fields[k] = plainToFsValue(v);
  }
  return fields;
}

/// plainToFsFields'in tek bir DEĞER için kullandığı yardımcı — nesneler
/// (mapValue) için kendini rekürsif çağırır.
function plainToFsValue(v) {
  if (typeof v === 'number') return { integerValue: String(Math.trunc(v)) };
  if (typeof v === 'boolean') return { booleanValue: v };
  if (v === null || v === undefined) return { nullValue: null };
  if (Array.isArray(v)) {
    return { arrayValue: { values: v.map(plainToFsValue) } };
  }
  if (typeof v === 'object') {
    return { mapValue: { fields: plainToFsFields(v) } };
  }
...
/// 27.08.2026 eklendi — MANUEL SATIŞ AKIŞI: developer, kendi ürettiği
/// (Sitora'nın kendi form/AI akışından GEÇMEMİŞ) bir siteyi doğrudan bir
/// müşterinin Sitora hesabına hazır bir proje olarak yazar. Böylece
/// müşteri hiçbir formu görmez/doldurmaz — Projelerim'i açtığında site
/// hazır durur, sadece kendi eliyle "Yayınla" / "Domain Bağla" yapar
/// (yayınlama/domain akışları zaten var olan, uygulama içi butonlardır —
/// bu fonksiyon SADECE Firestore'daki proje dokümanını oluşturur, Cloudflare
/// tarafında HİÇBİR ŞEYE dokunmaz; Worker/R2/D1 kaydı ancak müşteri kendi
/// hesabından "Yayınla"ya bastığında, mevcut publish akışıyla oluşur).
///
/// Beklenen body alanları — Flutter tarafındaki SiteProject.toJson() ile
/// BİREBİR aynı şema (bkz. lib/models/site_project.dart):
///   name        (zorunlu, string)
///   mode        ('single' | 'multi', varsayılan 'single')
///   kind        (ProjectKind enum adı, örn. 'kuafor', 'genericBusiness';
///                boşsa 'site' — bkz. o enum'ın tüm değerleri için Flutter
///                kaynağı)
///   code        (mode='single' ise zorunlu — tam HTML)
///   files       (mode='multi' ise zorunlu — { "index.html": "...", ... })
///   activeFileName (mode='multi' için, opsiyonel)
///   formData    (opsiyonel — verilirse müşteri ileride "Düzenle" ile bu
///                projeyi tekrar formdan açabilir; verilmezse "Düzenle"
///                boş formla açılır, sorun değildir, sadece o kolaylığı
///                kaybeder)
///   watermarkRemoved     (opsiyonel, varsayılan true — manuel satışta
///                          zaten ücrete dahil edildiği varsayılır; panelde
///                          ELLE true/false seçilebilir, bkz.
///                          injectProjectFormHtml)
///   publishRightGranted  (opsiyonel, varsayılan true — aynı gerekçe)
///
///   editableInApp SABİT false — panelde seçenek OLARAK SUNULMAZ. Admin
///   enjeksiyonuyla gelen bir site müşterinin kendi formundan değil,
///   developer'ın elle hazırladığı HTML'den gelir; müşteri "Düzenle"ye
///   basarsa bu elle hazırlanmış içerik forma göre üretilen jenerik bir
///   HTML ile geri dönüşsüz değişir. Bu yüzden bu uçtan oluşturulan HER
///   proje editableInApp=false ile kaydedilir (bkz. lib/models/
///   site_project.dart ve preview_screen.dart > _openEditForm).
async function handleAdminProjectCreate(uid, body, env) {
  if (!env.FIREBASE_SERVICE_ACCOUNT_JSON) {
    return json({ error: 'not_configured', message: 'FIREBASE_SERVICE_ACCOUNT_JSON tanımlı değil.' }, 501);
  }
  const name = (body.name || '').trim();
  if (!name) return json({ error: 'name zorunlu' }, 400);

  const mode = body.mode === 'multi' ? 'multi' : 'single';
  const kind = (body.kind || 'site').trim();

  if (mode === 'single' && !(body.code || '').trim()) {
    return json({ error: 'mode="single" için code zorunlu (tam HTML)' }, 400);
  }
  if (mode === 'multi' && (!body.files || typeof body.files !== 'object' || Object.keys(body.files).length === 0)) {
    return json({ error: 'mode="multi" için files zorunlu ({ "index.html": "..." } şeklinde)' }, 400);
  }
  ...
  const doc = {
    ...
    activeFileName: body.activeFileName || null,
    createdAt: now,
    updatedAt: now,
    watermarkRemoved: typeof body.watermarkRemoved === 'boolean' ? body.watermarkRemoved : true,
    publishRightGranted: typeof body.publishRightGranted === 'boolean' ? body.publishRightGranted : true,
    // Bilerek body'den OKUNMUYOR — panelden hiçbir zaman true gönderilemez,
    // bkz. yukarıdaki fonksiyon açıklaması.
    editableInApp: false,
  };
  if (body.formData && typeof body.formData === 'object') {
    doc.formData = body.formData;
  }
  ...
  // tarafı (SiteProject.fromJson) DateTime.parse ile okuyor, ikisi de
  // ISO 8601 string olduğu için timestampValue okuma tarafında sorunsuz.
  fields.createdAt = { timestampValue: doc.createdAt };
  fields.updatedAt = { timestampValue: doc.updatedAt };

  // Proje id'si: admin panelinden elle verilmediyse Firestore otomatik
  // üretsin (POST .../projects, id belirtilmeden) — Flutter tarafı zaten
  // id'yi dokümanın kendi adından okuyor (bkz. handleAdminUserProjects).
  const customId = (body.id || '').trim();
  const url = customId
    ? `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/users/${uid}/projects?documentId=${encodeURIComponent(customId)}`
    : `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/users/${uid}/projects`;
...
async function handleAdminSites(request, env) {
  ...
  // ile filtrelenebilir — panelde LIMIT'i aşan hesaplarda belirli bir siteyi
  // bulmak için (bkz. ADMIN_HTML > siteSearchInput/siteStatusFilter).
  // ?offset= ile sayfalama: "Daha fazla yükle" butonu her tıklamada offset'i
  // LIMIT kadar artırarak aynı search/status filtresiyle tekrar çağırır
  // (bkz. ADMIN_HTML > loadMoreSites). total da dönülür ki panelde "X / Y
  // gösteriliyor" yazılabilsin.
  //
  // 27.08.2026: LIMIT 200 -> 5000 yapıldı (kullanıcı isteği: "sınırsız
  // artır"). GERÇEKTEN sınırsız (LIMIT'siz) bırakılmadı — tek bir D1
  // sorgusunda binlerce/onbinlerce satırı response'a basmak, site sayısı
  // ileride gerçekten çok büyürse (ör. 50.000+) Worker'ın CPU süresi ve
  // response boyutu limitlerine takılıp panelin tamamen çökmesine yol
  // açabilir. 5000, "pratikte tek tıkla hepsini gör" hissini verirken
  // (yakın/orta vadede site sayısı bunun çok altında kalacaktır) bu riski
  // almıyor — offset/"Daha fazla yükle" altyapısı da bilerek AYNEN
  // korundu, ileride site sayısı 5000'i gerçekten geçerse panel yine
  // sorunsuz çalışmaya devam eder, sadece 2. sayfaya geçmen gerekir.
  const url = new URL(request.url);
...
async function loadSummary() {
  ...
  el.innerHTML =
    stat(s.totalSites, 'toplam site (tüm zamanlar)') +
...
/// 27.08.2026 eklendi — manuel satış akışı: developer'ın kendi ürettiği bir
/// siteyi (form akışı DIŞINDA) doğrudan bu kullanıcının hesabına "hazır
/// proje" olarak yazmasını sağlayan basit form.
///
/// Kopyala/yapıştır YOK — dosya yükleniyor: tek sayfa için .html/.htm,
/// çok sayfalı (veya css/js/görsel içeren) siteler için .zip. Ayrıştırma
/// tarayıcıda (JSZip ile, bkz. <head> içindeki cdnjs script'i) yapılıyor —
/// worker'a yine sadece düz JSON (code veya files) gidiyor,
/// handleAdminProjectCreate HİÇ değişmedi.
function injectProjectFormHtml(uid) {
  const idName = 'inj_name_' + uid;
  const idKind = 'inj_kind_' + uid;
  const idFile = 'inj_file_' + uid;
  const idWatermark = 'inj_wm_' + uid;
  const idStatus = 'inj_status_' + uid;
  return '<details style="margin-top:10px">' +
    '<summary style="cursor:pointer;font-size:13px;color:#9db">Hazır site enjekte et (manuel satış)</summary>' +
    '<div class="field-row"><label>Site adı</label><input type="text" id="' + idName + '" placeholder="ör. Ayşe Kuaför" /></div>' +
    '<div class="field-row"><label>Sektör (kind)</label><input type="text" id="' + idKind + '" placeholder="ör. kuafor, genericBusiness" /></div>' +
    '<div class="field-row"><label>Dosya</label><input type="file" id="' + idFile + '" accept=".html,.htm,.zip" /></div>' +
    '<div style="font-size:11px;color:#789;margin-top:2px">Tek sayfa: .html dosyası. Çok sayfalı (veya görsel/css/js içeren) siteler: hepsini bir .zip içine koyup yükle — kökte bir index.html olmalı.</div>' +
    '<div class="field-row" style="margin-top:6px">' +
    '<label><input type="checkbox" id="' + idWatermark + '" checked style="width:auto;margin-right:6px" />Rozet kaldırılsın (işaretli değilse site rozetli eklenir)</label>' +
    '</div>' +
    '<div style="margin-top:6px">' +
    '<button class="btn" onclick="injectProject(' + JSON.stringify(uid) + ')">Projeyi Oluştur</button>' +
    ' <span id="' + idStatus + '" style="font-size:12px;color:#789;margin-left:6px"></span>' +
    '</div>' +
    '<div style="font-size:12px;color:#789;margin-top:4px">Yayın hakkı her zaman verilmiş olarak eklenir. Bu proje uygulama içinde asla "Düzenle" ile açılamaz (elle hazırlanmış içerik korunur) — kullanıcı zip\\'i değil, sadece hazır siteyi görür.</div>' +
    '</details>';
}

/// Yüklenen dosyayı (.html/.htm ya da .zip) inceleyip worker'ın beklediği
/// { mode, code } ya da { mode, files, activeFileName } şekline çevirir.
/// ZIP içindeki klasör girdileri ve metin olarak okunamayan (muhtemelen
/// ikili — png/jpg gibi) dosyalar ATLANIR; bu admin aracı şu an sadece
/// HTML/CSS/JS gibi metin dosyalarını destekliyor (görseller zaten
/// sitelerde çoğunlukla data-URI/base64 olarak HTML'in içine gömülü
/// geliyor, bkz. Flutter tarafındaki gallery_picker_field.dart).
async function _readInjectedFile(file) {
  const isZip = /\\.zip$/i.test(file.name);
  if (!isZip) {
    const code = await file.text();
    return { mode: 'single', code };
  }
  ...
  for (const entry of entries) {
    // Zip genelde tek bir üst klasörle paketlenir (ör. "site/index.html") —
    // Flutter tarafı düz dosya adları bekliyor (bkz. site_project.dart >
    // files: Map<String,String>), bu yüzden en üst klasör varsa atılır.
    const parts = entry.name.split('/').filter(Boolean);
    const relName = parts.length > 1 ? parts.slice(1).join('/') : parts[0];
    if (!relName) continue;
    try {
      const text = await entry.async('string');
      // Kaba bir ikili-dosya tespiti: çözülen metinde NUL byte ya da çok
      // sayıda "replacement character" varsa muhtemelen bu bir resim/font
      // gibi ikili dosyadır — metin haritasına bozuk şekilde eklemek yerine
      // atlanır (admin'e uyarı olarak gösterilir).
  ...
  if (Object.keys(files).length === 0) {
    throw new Error('Zip içinde okunabilir bir dosya bulunamadı.');
  }
  // activeFileName: kökte index.html varsa o, yoksa ilk .html dosyası.
  let activeFileName = Object.keys(files).find((n) => n.toLowerCase() === 'index.html')
    || Object.keys(files).find((n) => /\\.html?$/i.test(n))
    || Object.keys(files)[0];
  return { mode: 'multi', files, activeFileName, skipped };
}

async function injectProject(uid) {
  const name = document.getElementById('inj_name_' + uid).value.trim();
  const kind = document.getElementById('inj_kind_' + uid).value.trim() || 'genericBusiness';
  const fileInput = document.getElementById('inj_file_' + uid);
  const watermarkRemoved = document.getElementById('inj_wm_' + uid).checked;
  const statusEl = document.getElementById('inj_status_' + uid);
  const file = fileInput.files && fileInput.files[0];
  if (!name) { alert('Site adı zorunlu.'); return; }
  if (!file) { alert('Bir .html ya da .zip dosyası seçmelisin.'); return; }

  statusEl.textContent = 'Okunuyor…';
  let parsed;
  try {
    parsed = await _readInjectedFile(file);
  } catch (e) {
    statusEl.textContent = '';
    alert('Dosya okunamadı: ' + (e && e.message ? e.message : String(e)));
    return;
  }
  if (parsed.skipped && parsed.skipped.length) {
    statusEl.textContent = '';
    const proceed = confirm(
      'Şu dosyalar metin olarak okunamadı (muhtemelen görsel/ikili) ve ATLANACAK:\\n' +
      parsed.skipped.join('\\n') +
      '\\n\\nBu dosyalar olmadan devam edilsin mi?'
    );
    if (!proceed) return;
  }

  if (!confirm('"' + name + '" adıyla bu kullanıcının hesabına hazır bir proje eklenecek. Devam edilsin mi?')) {
    statusEl.textContent = '';
    return;
  }
  statusEl.textContent = 'Yükleniyor…';
  const body = { name, kind, mode: parsed.mode, watermarkRemoved };
  if (parsed.mode === 'single') {
    body.code = parsed.code;
  } else {
    body.files = parsed.files;
    body.activeFileName = parsed.activeFileName;
  }
  const res = await api('/admin/users/' + encodeURIComponent(uid) + '/projects', {
    method: 'POST',
    body: JSON.stringify(body),
  }).catch((e) => ({ error: String(e) }));
  statusEl.textContent = '';
  if (res.error) { alert('Hata: ' + (res.message || res.error)); return; }
  alert('Proje oluşturuldu. Müşteri artık kendi hesabından Projelerim\\'de görebilir.');
  await loadUserProjects(uid);
}

async function loadUserProjects(uid) {  const el = document.getElementById('projects_' + uid);
...
// Sayfalama durumu: /admin/sites her seferinde en fazla 5000 site döner
// (bkz. handleAdminSites > LIMIT).
