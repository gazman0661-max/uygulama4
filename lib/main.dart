...
void main() {
  ...
  runZonedGuarded<void>(() async {
    ...
    // 06.09.2026 eklendi — bkz. services/server_time_service.dart: bir
    // önceki oturumdan kalan cihaz-saati/sunucu-saati sapmasını (varsa)
    // SharedPreferences'tan yükler. AppState._loadFromPrefs (birazdan
    // MultiProvider içinde create ile tetiklenecek) domain/mini paket süre
    // kontrollerinde ServerTimeService.now()'ı okuyacağı için bundan ÖNCE
    // tamamlanmış olması gerekiyor. Sadece yerel bir SharedPreferences
    // okuması olduğu için açılışı gözle görülür şekilde geciktirmez.
    ...
    try {
      ...
      // GÜVENLİK (20.09.2026 eklendi — düşük öncelik) — bkz. pubspec.yaml >
      // firebase_app_check yorumu. activate() BAŞARISIZ olsa bile (ör. Play
      // Integrity API konsolda henüz açılmadıysa) burası hata fırlatmaz —
      // sadece App Check token'sız devam eder, uygulama normal çalışır.
      // Enforce edilip edilmeyeceği TAMAMEN Firebase Console tarafında
      // (App Check sekmesi) belirlenir, burada değil.
      try {
    ...
    // sheet'leri) ilk karede zaten doğru isAvailable durumunu
    ...
    // Eski "aylık puan kotası" bildirimi (puan sistemiyle birlikte
    // kaldırıldı) cihazda hâlâ kurulu kalmış olabilir, açılışta iptal
    // ediliyor; pasif kullanıcı hatırlatması KADEMELİ bir seri (3/7/14/30
    // gün) — her açılışta baştan kurulur (kullanıcı sık açtıkça hiç
    // görünmez, uğramazsa sırasıyla 3, 7, 14 ve 30. günün öğleninde birer
    // hatırlatma gelir).
    unawaited(() async {
