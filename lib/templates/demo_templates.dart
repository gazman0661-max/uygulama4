...
/// Örnek görseller — kendi assetimiz yok, placehold.co zaten bu projede
/// başka yerlerde de (kapak görseli boşsa) varsayılan olarak kullanılıyor
/// (bkz. kuafor_form_screen.dart > coverImage fallback).
const _demoCover = 'https://placehold.co/1200x800?text=Kapak+G%C3%B6rseli';
const _demoGalleryUrls = [
...
final TemplateDemoConfig demoKuafor = TemplateDemoConfig(
  ...
  buildHtml: ({required themeId, required fontPackageId, required density}) => generateBusinessSiteHtml(
    ...
    gallery: _demoGalleryUrls.map((u) => {'url': u, 'caption': null}).toList(),
    workingHours: const [
