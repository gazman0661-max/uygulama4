...
/// 05.09.2026 eklendi (kanka isteği) — "serbest yerleşim": kullanıcı ilan
/// detay sayfasında TEK bir fotoğrafı, galeriyle gruplamadan, sayfanın
/// herhangi bir yerine (ör. Harita'nın altına) kendi bağımsız bölümü
/// olarak yerleştirebilsin diye (bkz. real_estate_form_screen.dart >
/// _photoLayoutOrder, real_estate_html_generator.dart > freeformLayout).
/// Aynı lightbox/tıkla-büyüt davranışını (gallery-lightbox) kullanır,
/// sadece TEK görsel için, galeri ızgarasına bağlı olmadan tam genişlikte
/// basılır.
String singlePhotoBlockHtml({required String url, String? caption}) {
  final captionHtml = (caption != null && caption.trim().isNotEmpty)
      ? '<p class="solo-photo-caption">${escapeHtml(caption)}</p>'
      : '';
  return '''
<section class="section solo-photo-section">
  <img src="${escapeHtml(url)}" alt="" onclick="document.getElementById('${_idFor(url)}').classList.add('open')">
  $captionHtml
...
String propertyCardGridHtml({
  ...
  final cards = listings.map((l) {
    // 05.09.2026 değişti (kanka isteği) — kapak fotoğrafı yoksa (ilanın
    // galerisi boşsa) ARTIK kırık/gri bir placeholder kutu basılmıyor.
    // <img> etiketi TAMAMEN atlanıyor, kart metin-öncelikli, temiz bir
    // görünüme düşüyor (bkz. CSS'teki .property-card.no-image kuralı).
    final hasImage = (l['coverImage'] ?? '').trim().isNotEmpty;
    final imgHtml = hasImage ? '<img src="${escapeHtml(l['coverImage']!)}" alt="">' : '';
    return '''
    <a class="property-card${hasImage ? '' : ' no-image'}" href="${escapeHtml(l['href'] ?? '#')}">
      $imgHtml
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .gallery-dot { width: 8px; height: 8px; border-radius: 50%; background: $border; display:inline-block; transition: width 0.25s ease, background 0.25s ease; }
  .solo-photo-section img { width:100%; max-height: 420px; object-fit: cover; border-radius: 14px; cursor: pointer; display:block; }
  .solo-photo-caption { margin-top: 8px; font-size: 13px; color: $subtext; }
  .gallery-dot.active { width: 22px; border-radius: 4px; background: $accent; }
  ...
  .property-card.no-image .property-card-body { padding-top:20px; }
