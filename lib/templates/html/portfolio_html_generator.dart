...
String generatePortfolioHtml({
  ...
  if (contactGoogleReviewLink != null && contactGoogleReviewLink.trim().isNotEmpty) {
    body.writeln('<div style="text-align:center;margin-top:8px;"><a class="contact-btn review" target="_blank" href="${escapeHtml(contactGoogleReviewLink.trim())}">${escapeHtml(siteLabels(lang)['googleReviewButton']!)}</a></div>');
  }
...
Map<String, String> generatePortfolioSite({
  ...
  if (contactGoogleReviewLink != null && contactGoogleReviewLink.trim().isNotEmpty) {
    homeBody.writeln('<div style="text-align:center;margin-top:8px;"><a class="contact-btn review" target="_blank" href="${escapeHtml(contactGoogleReviewLink.trim())}">${escapeHtml(siteLabels(lang)['googleReviewButton']!)}</a></div>');
  }
...
<section class="section" style="text-align:center; padding-bottom:0;">
  <a href="index.html" style="text-decoration:none; font-weight:600; font-size:14px;">$backToPortfolioText</a>
    ...
    if (contactGoogleReviewLink != null && contactGoogleReviewLink.trim().isNotEmpty) {
      workBody.writeln('<div style="text-align:center;margin-top:8px;"><a class="contact-btn review" target="_blank" href="${escapeHtml(contactGoogleReviewLink.trim())}">${escapeHtml(siteLabels(lang)['googleReviewButton']!)}</a></div>');
    }
