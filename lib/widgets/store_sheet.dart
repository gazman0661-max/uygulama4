...
import '../widgets/mini_package_purchase_sheet.dart';
...
class _StoreSheetState extends State<_StoreSheet> {
  ...
  /// 05.09.2026 eklendi (kanka isteği) — "1 Aylık Mini Paket" için proje
  /// seçtirir, sonra showMiniPackagePurchaseSheet'i açar. Rozet kaldırmadan
  /// FARKI: burada TÜM projeler listelenir (watermark'ın aksine tek
  /// seferlik/kalıcı değil, süresi dolan bir proje için tekrar/yenilenerek
  /// satın alınabilir) — activateMiniPackage her çağrıda süreyi yeniden 1
  /// aya ayarladığı için zaten aktif bir projede tekrar satın almak da
  /// zararsızdır (bilinçli bir "erken yenileme" olarak davranır).
  Future<void> _pickProjectForMiniPackage() async {
    final appState = context.read<AppState>();
    ...
    await showMiniPackagePurchaseSheet(context, project: selected);
  }
  ...
  Widget build(BuildContext context) {
    ...
    // 05.09.2026 eklendi — "1 Aylık Mini Paket" satırı, watermark satırıyla
    // AYNI desen: site seçmeden ÖNCE fiyatı burada gösteriyoruz.
    final miniPackagePriceLabel =
        BillingService.instance.products[kProductMiniPackage]?.price ?? t(context, 'Fiyat alınamadı');
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              child: Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      en
                          ? 'Connecting a domain and the Mini Package both remove the badge AND unlock: lead inbox, full gallery, map, lead form, Google review button, visitor stats. To use these, open the relevant site from My Projects and update it via the "Edit" button.'
                          : 'Domain bağlama ve Mini Paket, rozeti kaldırmanın YANI SIRA şunları da açar: Talep Kutusu, tam galeri, harita, talep formu, Google yorum butonu, ziyaretçi sayısı. Bunları kullanmak için ilgili siteyi Projelerim ekranından açıp "Düzenle" butonuyla güncellemen gerekir.',
                      style: const TextStyle(
            ...
            _StoreRow(
              emoji: '🎟️',
              title: t(context, '1 Aylık Mini Paket'),
              subtitle: en
                  ? 'Site-specific, 1 month — removes the badge AND unlocks other premium features (download not included). One-time purchase, does NOT auto-renew — locks return automatically if you don\'t buy it again. Price: $miniPackagePriceLabel'
                  : 'Site bazlıdır, 1 aylık — rozeti kaldırır VE diğer premium özellikleri açar (indirme dahil değildir). Tek seferlik satın almadır, OTOMATİK YENİLENMEZ — tekrar satın almazsan kilitler otomatik geri gelir. Fiyat: $miniPackagePriceLabel',
              trailingLabel: t(context, 'Site Seç'),
              onTap: _pickProjectForMiniPackage,
            ),
            if (_error != null) ...[
              const SizedBox(height: 12),
              Text(
