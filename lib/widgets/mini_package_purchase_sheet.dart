...
class _MiniPackagePurchaseSheetState extends State<_MiniPackagePurchaseSheet> {
  ...
  /// 05.09.2026 eklendi (kanka isteği) — remove_watermark_sheet.dart'taki
  /// _redeemGift ile AYNI desen. Mini paket domain'in aksine tamamen
  /// yerel olduğu için (worker'a hiç istek atmıyor) redeemGiftMiniPackage
  /// hem bakiyeyi düşürür hem activateMiniPackage'i kendi içinde çağırır —
  /// burada SADECE sonucu _done'a yansıtmak yeterli.
  Future<void> _redeemGift() async {
    setState(() {
      _busy = true;
      _error = null;
    });
    final ok = await context.read<AppState>().redeemGiftMiniPackage(widget.project.id);
    if (!mounted) return;
    if (!ok) {
          ...
          children: [
            ...
            Text(
              _done
                  ...
                  : (en
                      ? '"$name" gets the "Made with MySitora" badge removed AND other premium features unlocked (lead inbox, full gallery, map, lead form, Google review button, visitor stats) for 1 MONTH — one-time, applies ONLY to this site. This is a one-time purchase, NOT an auto-renewing subscription — it will NOT renew or charge you again by itself; you must come back and buy it again to extend. Just like the custom domain package, EXCEPT download rights are NOT included — downloading still needs a separate purchase. If you don\'t renew before the month ends, the badge and locks come back automatically.'
                      : '"$name" bu satın almayla 1 AY boyunca "MySitora ile üretildi" rozeti kalkar VE diğer premium özellikler (Talep Kutusu, tam galeri, harita, talep formu, Google yorum butonu, ziyaretçi sayısı) açılır — tek seferlik, SADECE bu site için geçerlidir. Bu tek seferlik bir satın almadır, OTOMATİK YENİLENEN bir abonelik DEĞİLDİR — kendiliğinden yenilenmez, tekrar ücret kesmez; süresi dolmadan uzatmak istersen tekrar senin satın alman gerekir. Özel domain paketiyle AYNI şekilde çalışır, TEK FARKI indirme hakkının DAHİL OLMAMASI — indirmek için ayrıca satın alma gerekir. Süresi dolmadan yenilemezsen rozet ve kilitler otomatik geri gelir.'),
              style: const TextStyle(
            ...
            if (!_done && context.watch<AppState>().giftMiniPackageCredits > 0) ...[
              SizedBox(
                ...
                child: PillButton(
                  label: _busy
                      ...
                      : (en
                          ? 'Use gift credit (${context.watch<AppState>().giftMiniPackageCredits} left)'
                          : 'Hediye hakkını kullan (${context.watch<AppState>().giftMiniPackageCredits} adet)'),
                  emoji: _busy ? null : '🎁',
