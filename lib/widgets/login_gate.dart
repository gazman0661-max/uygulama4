...
class _LoginGateSheetState extends State<_LoginGateSheet>
  ...
  void initState() {
    ...
    // 31.08.2026 eklendi — App Links: doğrulama linki artık uygulamayı
    // DOĞRUDAN açabiliyor (bkz. auth_link_service.dart), yani kullanıcı
    // hiç "arka plana alıp geri dönmeyebilir" (didChangeAppLifecycleState
    // hiç tetiklenmeyebilir). Bu yüzden AuthLinkService'in "az önce
    // doğrulandı" sinyalini de AYRICA dinliyoruz.
    AuthLinkService.instance.emailVerifiedPulse.addListener(_onLinkPulse);
  ...
  void _onLinkPulse() {
    if (_awaitingVerification) _checkVerified();
  }

  @override
  void dispose() {
    WidgetsBinding.instance.removeObserver(this);
    AuthLinkService.instance.emailVerifiedPulse.removeListener(_onLinkPulse);
    _emailCtrl.dispose();
