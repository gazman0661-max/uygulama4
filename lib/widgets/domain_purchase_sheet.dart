...
class _DomainPurchaseSheetState extends State<_DomainPurchaseSheet> {
  ...
  Future<void> _purchase() async {
    // 20.09.2026: await'ten ÖNCE yakalanır — sheet ödeme sırasında kapansa
    // bile +1 yayın hakkı (aşağıda) yine de hesaba yazılır.
    ...
    try {
      ...
      // 20.09.2026 eklendi (kanka isteği): "Kendi Domainimi Bağla" (1 yıllık)
      // artık +1 ek site yayın hakkı da içerir. SADECE gerçek ödemede
      // (buyConsumable true) verilir — hediye hakkı (_redeemGift) bu yoldan
      // GEÇMEZ, yani hediye ile bağlayan ek yayın hakkı almaz. Hem ilk
      // bağlamada hem yenilemede (aynı ürün) verilir. Bkz. billing_constants
      // > kProductConnectDomain. Ödeme zaten alındı: bakiye yazımı bir
      // sebeple hata verse bile akışı bozma (bellekteki bakiye artmış olur).
      try {
  ...
  /// publish_paywall_sheet.dart) AYNI desen.
  String _priceLabel() {
          ...
          children: [
            ...
            Text(
              isRenewal
                  ...
                  : (en
                      ? '"$name" gets its own domain for 1 year with this purchase — one-time, applies ONLY to this site. This is a one-time purchase, NOT an auto-renewing subscription — it will NOT renew or charge you again by itself; you must come back and buy it again to extend. The "Made with MySitora" badge is removed for as long as the domain stays connected — if the domain lapses and isn\'t renewed, the badge comes back automatically (this is not a permanent removal). Other premium features (lead inbox, full gallery, map, lead form, Google review button + Google Business Profile Setup Wizard, visitor stats) unlock for as long as the domain stays connected — extending before it expires requires this SAME purchase again, and if it lapses for more than 7 days the connection is fully released and this purchase must be made again to reconnect. This purchase also adds +1 extra site publish right to your account (usable for any new site).'
