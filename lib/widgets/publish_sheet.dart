...
/// zaten [requireLogin] ile giriş kontrolünü yapmış olmalı — bu widget'ın
/// kendisi girişten habersizdir, sadece HostingService.publish'i sarmalar.
///
...
Future<void> showPublishSheet(
  ...
  String? ownerEmail,
  required Future<void> Function(String subdomain, String url) onPublished,
}) {
...
class _PublishSheet extends StatefulWidget {
  ...
  final Future<void> Function(String subdomain, String url) onPublished;
...
class _PublishSheetState extends State<_PublishSheet> {
  ...
  Future<void> _publish() async {
    ...
    try {
      ...
      if (!mounted) return;
      await widget.onPublished(result.subdomain, result.url);
      if (!mounted) return;
  ...
  void _copy(String value) {
    ...
    ScaffoldMessenger.of(context)
        .showSnackBar(SnackBar(content: Text(t(context, 'Kopyalandı ✅'))));
  }
