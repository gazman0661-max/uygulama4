...
class _BuilderScreenState extends State<BuilderScreen> with WidgetsBindingObserver {
  ...
  void _undo() {
    if (_undoStack.isEmpty) {
      _snack('Geri alınacak bir değişiklik yok.');
      return;
    }
  ...
  void _redo() {
    if (_redoStack.isEmpty) {
      _snack('İleri alınacak bir değişiklik yok.');
      return;
    }
    _undoStack.add(_cloneAll());
  ...
  /// index.html > buildHTML(): "font-size: clamp(12px, Xvw, 72px)". Editör
  ...
  /// genişliğine ORANLA bir "vw" değeri hesaplayıp clamp() ile alt/üst
  /// sınır koyuyoruz — böylece dışa aktarılan site HANGİ CİHAZDA açılırsa
  ...
  String _responsiveFontSize(double px) {
    ...
    final vw = (px / refW * 100).toStringAsFixed(2);
  ...
  String _buildExportHtml() {
    ...
    final buffer = StringBuffer()
      ...
      ..writeln('body{margin:0;font-family:-apple-system,Segoe UI,Roboto,sans-serif;'
          'background:#0D0D0D;display:flex;justify-content:center;}')
      ..writeln('.astro-canvas{position:relative;width:100%;max-width:480px;'
          'aspect-ratio:9/17;background:${_colorToHex(_canvasBg)};overflow:hidden;}')
  ...
  Future<void> _confirmClearCanvas() async {
    if (_elements.isEmpty) {
      _snack('Canvas zaten boş.');
      return;
    }
    final confirmed = await showConfirmPopup(
      context,
      title: 'Tasarımı Sil',
      message: 'Canvas üzerindeki tüm elemanlar silinecek. Bu işlem geri alınamaz. '
    ...
    _snack('Tasarım silindi.', icon: '🗑️');
  }

  Future<void> _exportAndDownload() async {
    if (_elements.isEmpty) {
      _snack('Canvas boş — önce bir şablon seçiniz ya da eleman ekleyiniz.');
      return;
    }
    try {
      ...
      // (bkz. AppState.costSinglePage). SADECE bu canvas'ın İLK export'unda
      // (_savedProjectId henüz null'sa, yani YENİ bir "Projelerim" kaydı
      // açılacaksa) puan harcanır — aynı tasarımı tekrar tekrar indirmek
      // (mevcut projeyi güncellemek) form akışında da ücretsiz olduğu gibi
      // burada da ücretsizdir.
      ...
      if (saved) {
        _snack('HTML dosyası cihaza kaydedildi ve Projelerim\'e eklendi! 💾', icon: '✅');
      } else {
        ...
        _snack('Projelerim\'e kaydedildi (dosya indirilmedi).', icon: '📁');
      }
    } catch (e) {
        ...
        child: FittedBox(
          ...
          child: Row(
            ...
            children: [
              ...
              _topToolBtn(icon: Icons.redo, label: 'İleri', onTap: _redo),
              _topToolBtn(
                icon: Icons.grid_on,
                label: 'Izgara',
                active: _gridOn,
  ...
  Widget _buildBottomToolbar() {
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: [
            _toolChip(emoji: '📑', label: 'Şablonlar', onTap: _openTemplatesSheet),
            _toolChip(emoji: '✏️', label: 'Çizim', onTap: _showDrawingDialog),
            _toolChip(emoji: '🧱', label: 'Bölümler', onTap: _showSectionMenu),
            _toolChip(emoji: '📐', label: 'Şekiller', onTap: _showShapeMenu),
            _toolChip(
              emoji: '⚡',
              label: 'Smart CTA',
              onTap: () => _addButtonElement(label: t(context, 'Hemen İncele'), color: AppColors.accentBlue),
            ),
            _toolChip(emoji: '📝', label: 'Yazı', onTap: _addTextElement),
