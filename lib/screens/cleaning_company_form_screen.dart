...
/// ve metinler Temizlik Şirketi Sitesi için özelleştirildi.
class CleaningCompanyFormScreen extends StatefulWidget {
  const CleaningCompanyFormScreen({super.key, this.initialData, this.isEditing = false});
  ...
  @override
  State<CleaningCompanyFormScreen> createState() => _CleaningCompanyFormScreenState();
}

class _CleaningCompanyFormScreenState extends State<CleaningCompanyFormScreen> {
  ...
  Future<void> _generate() async {
    ...
    final ok = await LocalGenerationHelper.generateSinglePage(
      ...
      kind: ProjectKind.cleaningCompany,
      formData: formData,
  ...
  Future<void> _openAiFillDialog() async {
    final result = await showAiFillDialog(
      context: context,
      sectorLabel: t(context, 'Temizlik Şirketi'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Temizlik Şirketi Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Temizlik Şirketi Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: t(context, 'Ev Temizliği - 3 saat - 500 TL\nOfis Temizliği - 800 TL'),
