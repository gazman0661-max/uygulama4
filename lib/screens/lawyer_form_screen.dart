...
/// metinler Avukat / Hukuk Bürosu Sitesi için özelleştirildi.
class LawyerFormScreen extends StatefulWidget {
  const LawyerFormScreen({super.key, this.initialMultiPage = false, this.initialData, this.isEditing = false});
  ...
  @override
  State<LawyerFormScreen> createState() => _LawyerFormScreenState();
}

class _LawyerFormScreenState extends State<LawyerFormScreen> {
  ...
  Future<void> _openAiFillDialog() async {
    final result = await showAiFillDialog(
      context: context,
      sectorLabel: t(context, 'Avukat / Hukuk Bürosu'),
      initialCompanyName: _businessNameCtrl.text,
    ...
    setState(() {
      if (result['companyName'] != null) _businessNameCtrl.text = result['companyName']!;
      ...
      if (result['bio'] != null) _bioCtrl.text = result['bio']!;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Avukat / Hukuk Bürosu Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Avukat / Hukuk Bürosu Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(labelText: t(context, 'İsim / büro adı'), border: OutlineInputBorder()),
            ),
            const SizedBox(height: 12),
            TextField(
              controller: _titleCtrl,
              decoration: InputDecoration(labelText: t(context, 'Unvan (örn. Avukat)'), border: OutlineInputBorder()),
            ),
            const SizedBox(height: 12),
            TextField(
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: t(context, 'Ticaret Hukuku\nAile Hukuku\nİş Hukuku'),
