...
Future<void> openLeadInboxScreen(BuildContext context) async {
  ...
  AnalyticsService.logLeadInboxOpened(locked: !hasPremium);
  if (!hasPremium) {
    final en = isEnglish(context);
    await showPremiumLockedPopup(
      context,
      message: en
          ? 'Requests from your site\'s contact form are available with a subscription or a custom-domain package (locked on the free plan).'
          : 'Sitenin talep formundan gelen talepleri görebilmek abonelik veya özel domain paketinde açılır (ücretsiz planda kilitli).',
    );
    return;
  }
