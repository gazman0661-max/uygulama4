...
import '../widgets/rewarded_ad_popup.dart';
  ...
  Future<void> _openEditForm() async {
    ...
    if (!appState.qtCurrentEditableInApp) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(t(context, 'Bu site bu hesaba özel hazırlanmıştır ve uygulama içinden düzenlenemez.'))),
      );
      return;
    }
    ...
    if (kind == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(t(context, 'Bu site formdan düzenlenemiyor.'))),
      );
      return;
    }
    ...
    if (screen == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(t(context, 'Bu site türü için düzenleme ekranı yok.'))),
      );
      return;
    }
...
''';
  ...
  ///
  /// 26.08.2026 eklendi — İNDİRMEDEN ÖNCE rewarded reklam onay popup'ı
  /// (bkz. widgets/rewarded_ad_popup.dart): kullanıcı "Vazgeç" derse ya da
  /// reklamı yarıda bırakırsa asıl indirme BAŞLAMAZ, fonksiyon burada döner.
  Future<void> _downloadSite() async {
    ...
    final proceed = await showRewardedAdGate(
      context,
      message: 'İndirmek için kısa 1 reklam izle',
      placement: 'download',
    );
  ...
  Future<void> _publishSite() async {
    ...
    // 26.08.2026 eklendi — YAYINLAMADAN ÖNCE rewarded reklam onay popup'ı
    // (bkz. widgets/rewarded_ad_popup.dart). Ücretli yayın hakkı satın alma
    // akışından (paywall) SONRA gösterilir ki kullanıcı hem ödeyip hem de
    // reklam izlemek zorunda hissetmesin — paywall zaten geçildiyse (veya
    // gerek yoksa) burası son adımdır.
    final proceed = await showRewardedAdGate(
      context,
      message: 'Yayınlamak için kısa 1 reklam izle',
      placement: 'publish',
    );
    if (!proceed || !mounted) return;
            ...
            Expanded(
              child: FittedBox(
                ...
                children: [
                  ...
                  // (bkz. _openEditForm). Her "Düzenlemeyi Bitir" 2 puan
                  // düşer (bkz. AppState.costEdit).
                  //
