...
class _PreviewScreenState extends State<PreviewScreen> {
  ...
  bool get _isMulti => context.read<AppState>().siteMode == SiteMode.multi;
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          onPageFinished: (url) async {
            ...
            // çıkarıp appState.activeFileName'i güncelliyoruz ki AI
            ...
            if (appState.siteMode == SiteMode.multi) {
              ...
              if (name != null &&
                  name.isNotEmpty &&
                  appState.generatedFiles.containsKey(name)) {
                appState.setActiveFile(name);
              }
            }
  ...
  Future<void> _loadInitialContent(AppState appState) async {
    if (appState.siteMode == SiteMode.multi) {
      ...
      final fixedFiles = _ensureViewportMetaForFiles(appState.generatedFiles);
      final indexPath =
      ...
      final code = appState.generatedCode;
      final safeCode =
  ...
  String _currentCode(AppState appState) {
    if (appState.siteMode == SiteMode.multi) {
      ...
      return appState.generatedFiles[active] ?? '';
    }
    return appState.generatedCode;
  }

  /// AI ile üretilen yeni içeriği doğru yere yazar: A modunda appState.generatedCode,
  /// B modunda appState.generatedFiles[activeFileName] + diskteki önizleme dosyası
  /// (linkler bozulmasın diye diğer dosyalara dokunulmaz).
  Future<void> _persistCode(AppState appState, String newCode) async {
    if (appState.siteMode == SiteMode.multi) {
      ...
      appState.updateGeneratedCode(newCode);
    }
  }
...
''';
  ...
  /// günlük ücretsiz puan kotası kontrol edilir (1 düzenleme = 2 puan).
  /// Yetersizse custom popup'ı gösterir ve false döner.
  Future<bool> _checkQuotaOrShowPopup() async {
    final appState = context.read<AppState>();
    if (await appState.ensureQuotaFor(AppState.costEdit)) return true;
  ...
  Future<void> _applySectionEdit(dynamic idx, String sectionHtml, String request) async {
    ...
    try {
      ...
      if (appState.usesFreeQuota) {
        updatedHtml = await WorkerService.editSection(
          sectionHtml: sectionHtml,
          request: request,
        );
      } else {
        updatedHtml = await GeminiService.editSection(
      ...
      await appState.consumeQuota(AppState.costEdit);
...
''';
  ...
  Future<void> _openBackgroundEditDialog() async {
    final request = await showAiEditDialog(
      context: context,
      icon: '🎨',
      title: 'AI Arka Plan Düzenleyici',
  ...
  Future<void> _applyBackgroundEdit(String request) async {
    ...
    try {
      ...
      // içeriği gönderiliyor (varsa) — worker.js'in "bg_edit" tipi artık
      // own-key akışıyla (GeminiService.editBackground) BİREBİR AYNI
      // çalışıyor. Böylece "rengi koru, animasyon ekle" gibi istekler kör
      // tahmin yerine gerçek mevcut değere bakarak yanıtlanır.
      final currentBgCss = _extractCurrentBackgroundCss(_currentCode(appState));
      final String css;
      if (appState.usesFreeQuota) {
        css = await WorkerService.editBackground(
          request: request,
          currentBackgroundCss: currentBgCss,
        );
      } else {
        css = await GeminiService.editBackground(
          apiKey: appState.effectiveApiKey!,
          model: appState.selectedModel,
          request: request,
          currentBackgroundCss: currentBgCss,
      ...
      if (appState.siteMode == SiteMode.multi && _previewDirPath != null) {
        ...
        final active = appState.activeFileName ?? 'index.html';
