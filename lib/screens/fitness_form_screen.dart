...
class FitnessFormScreen extends StatefulWidget {
  const FitnessFormScreen({super.key, this.initialData, this.isEditing = false});
...
class _FitnessFormScreenState extends State<FitnessFormScreen> {
  ...
  Map<String, dynamic> _captureFormData() {
    return {
      ...
      'scheduleCtrl': _scheduleCtrl.text,
  ...
  void _restoreFromInitialData(Map<String, dynamic> d) {
    ...
    _scheduleCtrl.text = (d['scheduleCtrl'] as String?) ?? _scheduleCtrl.text;
  ...
  Future<void> _generate() async {
    if (_nameCtrl.text.trim().isEmpty || _phoneCtrl.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(t(context, 'Stüdyo adı ve telefon gerekli.'))),
      );
      return;
    }
  ...
  Future<void> _openAiFillDialog() async {
    final result = await showAiFillDialog(
      context: context,
      sectorLabel: t(context, 'Fitness Stüdyosu'),
    ...
    setState(() {
      ...
      if (result['tagline'] != null) _sloganCtrl.text = result['tagline']!;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Fitness Stüdyosu Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Fitness Stüdyosu Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(labelText: t(context, 'Stüdyo adı'), border: OutlineInputBorder()),
            ),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                labelText: t(context, 'Üyelik / hizmet paketleri (her satıra bir tane: Ad - Fiyat)'),
                hintText: t(context, 'Aylık Üyelik - 1200 TL\nGrup Dersi (tekli) - 150 TL'),
            ...
            TextField(
              ...
              maxLines: 6,
              decoration: InputDecoration(
                labelText: t(context, 'Ders programı (her satıra bir tane: Gün - Saat - Ders - Eğitmen)'),
                hintText: t(context, 'Pazartesi - 18:00 - CrossFit - Ahmet\nÇarşamba - 19:00 - Yoga - Elif'),
                border: OutlineInputBorder(),
              ),
            ),
            const SizedBox(height: 20),
