...
Future<void> showSubscriptionPlansScreen(BuildContext context) async {
  ...
  AnalyticsService.logPaywallShown(trigger: 'subscription_plans');
...
class _InfoBox extends StatelessWidget {
  final String text;
  // 18.09.2026 fix (kanka bug raporu — "abonelerin üst kısmındaki yazı
  // açık temada okunmuyor") — bu kutu önceden isDark'tan HABERSİZDİ, hep
  // beyaz-üstü-beyaz (Colors.white.withOpacity(0.05) zemin + white60 metin)
  // renkler kullanıyordu; açık temada bu neredeyse görünmez bir açık gri
  // metin/kutu olarak render oluyordu. Artık ekranın geri kalanıyla AYNI
  // isDark parametresini alıp koyu temada eski görünümü koruyor, açık
  // temada okunaklı koyu metin + hafif gri zemin kullanıyor.
  final bool isDark;
  const _InfoBox({required this.text, required this.isDark});

  @override
  Widget build(BuildContext context) {
    final boxBg = isDark ? Colors.white.withOpacity(0.05) : Colors.black.withOpacity(0.04);
    final boxBorder = isDark ? Colors.white24 : Colors.black12;
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              text,
              style: TextStyle(color: textColor, fontFamily: 'monospace', fontSize: 11.5, height: 1.35),
            ),
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _FeatureLine(subtleColor: subtleColor, text: en ? 'Lead inbox, gallery, map, review button + Google profile setup wizard' : 'Talep Kutusu, galeri, harita, yorum butonu + Google profil kurulum sihirbazı'),
          // 20.09.2026 eklendi (kanka isteği) — bkz. guide_dialog.dart adım 13
          // / cloudflare/worker > handleSetGoogleVerification. Domain bağlama
          // ZATEN üçü de dahil olduğu için ayrıca kota/tier bazlı bir şart
          // eklemeye gerek yok, tek satır yeterli.
          _FeatureLine(subtleColor: subtleColor, text: en ? 'Connect to Google Search Console (faster indexing)' : 'Google Search Console\'a bağlan (daha hızlı indekslenme)'),
          _FeatureLine(
            subtleColor: subtleColor,
