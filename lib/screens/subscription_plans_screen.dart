...
class _SubscriptionPlansScreenState extends State<SubscriptionPlansScreen> {
  ...
  Future<void> _buy(String productId) async {
    ...
    try {
      ...
      // yazılmış duruma göre tazeliyoruz.
      ...
      await showAppPopup(
        context,
        icon: '✅',
        message: en ? 'Subscription active ✅' : 'Abonelik aktif ✅',
      );
      // 17.09.2026 eklendi (kanka isteği — "paket düşürdüğünde otomatik
      // açılsın" fix'i). Paket DEĞİŞTİRME (Play Billing'in kendi "aboneliği
      // değiştir" akışı) de AYNI buySubscription/_buy yolunu kullanıyor
      // (bkz. dosya başı dokümanı) — yani bir üst paketten alt pakete
      // geçiş de buraya düşer. refreshSubscriptionStatus SONRASI kota
      // aşımı oluştuysa (yeni paketin siteQuota/domainQuota'sı, kota
      // içindeki mevcut site/domain sayısından azsa) kullanıcı Ana
      // Sayfa'ya dönmeyi beklemeden, TAM BURADA sheet açılır — bkz.
      // home_screen.dart > _maybeAutoOpenQuotaOverflowSheets (o da AYNI
      // aşımı Ana Sayfa'dayken yakalar, burası sadece daha erken/aynı
      // ekranda gösterir, ikisi çakışmaz çünkü sheet açılır açılmaz
      // hasXQuotaOverflow false'a düşer).
      if (!mounted) return;
