...
import '../widgets/ai_fill_dialog.dart';
...
/// ve metinler Kişisel Antrenör Sitesi için özelleştirildi.
class PersonalTrainerFormScreen extends StatefulWidget {
  const PersonalTrainerFormScreen({super.key, this.initialMultiPage = false, this.initialData, this.isEditing = false});
  ...
  @override
  State<PersonalTrainerFormScreen> createState() => _PersonalTrainerFormScreenState();
}

class _PersonalTrainerFormScreenState extends State<PersonalTrainerFormScreen> {
  ...
  Future<void> _generate() async {
    ...
    final ok = _multiPage
        ? await LocalGenerationHelper.generateMultiPage(
            ...
            kind: ProjectKind.personalTrainer,
            formData: formData,
  ...
  /// 30.08.2026 eklendi — "✨ AI ile Doldur" popup'ı (bkz. widgets/ai_fill_dialog.dart).
  Future<void> _openAiFillDialog() async {
    final result = await showAiFillDialog(
      context: context,
      sectorLabel: t(context, 'Kişisel Antrenör'),
      initialCompanyName: _nameCtrl.text,
      siteLang: _siteLang,
    );
    if (result == null || !mounted) return;
    setState(() {
      if (result['companyName'] != null) _nameCtrl.text = result['companyName']!;
      if (result['about'] != null) _aboutCtrl.text = result['about']!;
      if (result['skills'] != null) _skillsCtrl.text = result['skills']!;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Kişisel Antrenör Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Kişisel Antrenör Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
          ...
          children: [
            // 30.08.2026 eklendi — "✨ AI ile Doldur" (bkz. widgets/ai_fill_dialog.dart).
            Align(
              alignment: Alignment.centerRight,
              child: PillButton(
                label: t(context, 'AI ile Doldur'),
                emoji: '✨',
                borderColor: AppTheme.accentCyan,
                textColor: AppTheme.accentCyan,
                onTap: _openAiFillDialog,
              ),
            ),
            ...
            TextField(
              ...
              decoration: InputDecoration(labelText: t(context, 'Uzmanlık (örn. Kişisel Fitness Koçu)'), border: OutlineInputBorder()),
            ),
            const SizedBox(height: 12),
            TextField(
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: t(context, 'Kilo Verme, Kas Kütlesi Artırma, Online Koçluk'),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: t(context, '2023 - Kişisel Antrenör - Freelance\n2020 - Fitness Eğitmenliği Sertifikası - Federasyon'),
                border: OutlineInputBorder(),
              ),
            ),
            const SizedBox(height: 20),
            GalleryPickerField(
              label: t(context, 'Öncesi/Sonrası & Çalışma Fotoğrafları'),
