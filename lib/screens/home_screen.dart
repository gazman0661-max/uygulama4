  ...
  Widget _buildHomeTab(AppState appState) {
    final allCards = <_HomeToolCardData>[
      ...
      _HomeToolCardData(
        ...
        id: 'generic_business', // 19.09.2026 eklendi — bkz. _HomeToolCardData.id
    ...
    if (query.isEmpty) {
      ...
      if (filteredCards.isEmpty) {
        // 19.09.2026 düzeltildi — ESKİ kod burada `c.title == 'Genel
        // İşletme Sitesi'` diye ARIYORDU; title çeviriden geldiği için
        // (t(context, ...)) İngilizce dilde eşleşme hiç bulunamıyor ve
        // firstWhere (orElse yokken) "Bad state: No element." ile çöküyordu
        // (bkz. Crashlytics issue 9d6cfa96...). Artık dile bağlı olmayan
        // id ile arıyoruz, üstelik olası bir gelecekteki uyuşmazlıkta bile
        // çökmemesi için orElse ile GÜVENLİ bir geri dönüş veriyoruz.
        final fallback = allCards.firstWhere(
          (c) => c.id == 'generic_business',
...
class _HomeToolCardData {
  ...
  // 19.09.2026 eklendi — Crashlytics'te yakalanan "Bad state: No element."
  // çökmesinin düzeltmesi (bkz. _buildHomeTab > firstWhere). ESKİ kod
  // fallback kartı `c.title == 'Genel İşletme Sitesi'` diye ARAMA yapıyordu;
  // title çeviriden geldiği için (t(context, ...)) İngilizce dilde
  // "General Business Site" oluyor ve eşleşme hiç bulunamayıp firstWhere
  // (orElse yokken) StateError fırlatıyordu. Artık dile bağlı olmayan sabit
  // bir kimlikle ('generic_business') eşleştiriyoruz — bkz. satır ~316 ve
  // _buildHomeTab içindeki firstWhere çağrısı.
  final String? id;
