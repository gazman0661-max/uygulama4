...
import '../widgets/no_ad_popup.dart';
...
enum ChatMode { aiChat, builderPro }
...
class _HomeScreenState extends State<HomeScreen> {
  ChatMode _mode = ChatMode.aiChat;
    ...
    try {
      if (appState.usesFreeQuota) {
        ...
        if (appState.siteMode == SiteMode.single) {
          ...
          appState.updateGeneratedFiles(files);
          await appState.consumeQuota(AppState.costGenerateSite);
          setState(() {
            _messages.removeLast();
            _messages.add(ChatMessage(
  ...
  /// Üst panel: başlık + AI CHAT/BUILDER PRO seçici + 4 ikon butonu tek satırda.
  ///
  ...
  Widget _buildModeToggle() {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          _modeChip('AI CHAT', ChatMode.aiChat),
          _modeChip('BUILDER PRO', ChatMode.builderPro),
        ],
      ),
    );
  }

  Widget _modeChip(String label, ChatMode mode) {
    final selected = _mode == mode;
    return GestureDetector(
      onTap: () {
        if (mode == ChatMode.builderPro) {
          // BUILDER PRO kendi tam ekranı: sohbet moduna karışmadan ayrı
          // bir ekran olarak açılır, kapatınca AI CHAT'e geri dönülür.
          Navigator.of(context).push(
  ...
  /// "Tek Sayfa (Biolink)" / "Çok Sayfa (Zip)" — A/B modu seçici.
  /// Sadece görünüm/etiket değişmiyor: bu seçime göre _sendMessage() farklı
  /// bir GeminiService fonksiyonunu (generateSiteCode / generateMultiPageSite)
        ...
        children: [
          Expanded(
            child: _siteModeChip(
              ...
              title: 'Tek Sayfa',
              subtitle: 'Biolink / kartvizit — HTML indir',
            ),
          ),
          const SizedBox(width: 8),
          Expanded(
            child: _siteModeChip(
              ...
              mode: SiteMode.multi,
              title: 'Çok Sayfa',
              subtitle: 'Bağlantılı sayfalar — ZIP indir',
            ),
          ),
        ],
      ),
  ...
  Widget _siteModeChip({
    ...
    final selected = appState.siteMode == mode;
    return GestureDetector(
      onTap: () => appState.setSiteMode(mode),
      child: Container(
  ...
  /// İkinci satır: Kılavuz / Puan Ver / elmas sayacı / Reklam İzle.
  ///
  ...
  Future<void> _watchRewardedAd(AppState appState) async {
    setState(() => _adLoading = true);
    try {
      final rewarded = await AdService.showRewardedAd();
      if (!mounted) return;
      if (rewarded) {
        // Sadece reklam GERÇEKTEN sonuna kadar izlenip ödül sinyali
        // geldiyse puan eklenir.
        await appState.addCredits(5);
        if (!mounted) return;
        showAppPopup(context, message: '+5 puan eklendi! 🎬', icon: '🎉');
      } else {
        // Reklam bulunamadı/yüklenemedi/izlenmedi -> puan YOK, custom popup.
        await showNoAdPopup(context);
      }
    } finally {
  ...
  Widget _buildAdButton(AppState appState) {
    return Material(
      color: Colors.transparent,
      borderRadius: BorderRadius.circular(28),
      child: InkWell(
        borderRadius: BorderRadius.circular(28),
        onTap: _adLoading ? null : () => _watchRewardedAd(appState),
        child: Container(
          height: 46,
          padding: const EdgeInsets.symmetric(horizontal: 14),
          alignment: Alignment.center,
