...
import '../widgets/examples_gallery_dialog.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _sendMessage() async {
    ...
    // AI puan kotası kontrol edilir (tek sayfa = 5, çok sayfa = 10 puan).
    final _costForThisGen = appState.siteMode == SiteMode.single
        ? AppState.costSinglePage
        : AppState.costMultiPage;
    if (!await appState.ensureAiQuotaFor(_costForThisGen)) {
    ...
    try {
      // Tüm istekler Worker üzerinden yapılır — hem A hem B modu.
      if (appState.siteMode == SiteMode.single) {
        ...
        // Analitik: bkz. analytics_service.dart dosya başı NOT — bu
        // kaynak (ai_chat) hem yeni üretimi hem sohbet içi düzenlemeyi
        // kapsar (previousCode doluysa aslında bir düzenlemedir).
        ...
        await appState.consumeAiQuota(AppState.costMultiPage);
        unawaited(AnalyticsService.logSiteGenerated(
          source: 'ai_chat',
          mode: 'multi',
        ));
        ...
        child: FittedBox(
          ...
          child: Row(
            ...
            children: [
              ...
              // AI SOHBET sekmesi AppConfig.aiEditingEnabled ile açılıp
              // kapanıyor (bkz. lib/config/app_config.dart). Bayrak false
              // iken geçiş butonu HİÇ render edilmiyor, _activeTab hep
              // _MainTab.home'da sabit kalıyor — yani AI Chat'e (ve
              // dolayısıyla _buildBottomToolbar üzerinden EditScreen/AI
              // düzenlemeli PreviewScreen'e) hiçbir yoldan ulaşılamıyor.
              // Pro'ya geçince tek satır (AppConfig.aiEditingEnabled) ile
              // geri açılabilir.
  ...
  Widget _buildHomeSiteModeToggle(AppState appState) {
    return _siteModeToggleRow(
      ...
      singleSubtitle: t(context, 'Tek sayfalık şablonlar'),
      multiSubtitle: t(context, 'Çok sayfalı şablonlar (Emlak)'),
    );
  }

  /// AI Sohbet sekmesindeki "Tek Sayfa (Biolink) / Çok Sayfa (Zip)" — A/B
  /// modu seçici. Sadece görünüm/etiket değişmiyor: bu seçime göre
  /// _sendMessage() farklı bir WorkerService fonksiyonunu (generateSiteCode /
  ...
  Widget _buildChatSiteModeToggle(AppState appState) {
    return _siteModeToggleRow(
      ...
      singleSubtitle: t(context, 'Biolink / kartvizit — HTML indir'),
      multiSubtitle: t(context, 'Bağlantılı sayfalar — ZIP indir'),
    );
  }
        ...
        child: FittedBox(
          ...
          child: Row(
            ...
            children: [
              ...
              // Örnekler sadece AI Sohbet sekmesinde anlamlı (Hızlı Araçlar/
              // Ana Sayfa'da sohbet kutusu yok, dolduracak bir yer olmaz).
              if (_activeTab == _MainTab.chat) ...[
                PillButton(
                  label: t(context, 'Örnekler'),
                  emoji: '🎨',
                  ...
                  fontSize: 12.5,
                  padding: const EdgeInsets.symmetric(horizontal: 16),
                  onTap: () => showExamplesGalleryDialog(
                    context,
                    onPick: (prompt) {
  ...
  Widget _buildCreditChip(AppState appState) {
    // Aktif sekmeye göre doğru havuz gösterilir: ANA SAYFA (form) sekmesinde
    // FORM kotası (15), AI SOHBET sekmesinde AI kotası (20). Önceden bu chip
    // sekme farkı gözetmeden HER ZAMAN AI kotasını gösteriyordu; bu yüzden
    // form sekmesinde de yanlışlıkla 20 görünüyordu.
    final isChatTab = _activeTab == _MainTab.chat;
    final current = isChatTab ? appState.aiCredits : appState.formCredits;
    final max = isChatTab ? AppState.maxAiPoints : AppState.maxFormPoints;
    return Container(
      height: 38,
