...
class _EditScreenState extends State<EditScreen> {
  ...
  Future<void> _importFile() async {
    try {
      ...
      _relint(content);
      if (!mounted) return;
      showAppPopup(context, message: isEnglish(context) ? '"${picked.name}" imported.' : '"${picked.name}" içeri aktarıldı.', icon: '📂');
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _sendAiEdit(String request) async {
    ...
    if (!await appState.ensureQuotaFor(AppState.costEdit)) {
      await showQuotaLimitPopup(context);
      return;
    }

    // Not: Kendi anahtarıyla giden GeminiService.editFullCode BİREBİR aynı
    // fonksiyon/prompt — A ve B modu arasında farkeden tek şey hangi
    // "currentCode"un gönderildiği (A: generatedCode, B: editörde açık
    // aktif dosya). Worker akışında da worker.js'in kendi "full_edit"
    // tipi (gemini_service.dart'taki buildFullEditPrompt ile BİREBİR aynı
    // diff-tabanlı prompt) kullanılır — own-key akışıyla tam simetrik.
    setState(() => _aiBusy = true);
    try {
      final currentCode = _readCurrentCode(appState);
      final String updated;
      if (appState.usesFreeQuota) {
        updated = await WorkerService.editFullCode(
          currentCode: currentCode,
          request: request,
        );
      } else {
        updated = await GeminiService.editFullCode(
