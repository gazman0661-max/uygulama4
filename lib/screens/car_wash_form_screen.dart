...
class CarWashFormScreen extends StatefulWidget {
  const CarWashFormScreen({super.key, this.initialData, this.isEditing = false});
...
class _CarWashFormScreenState extends State<CarWashFormScreen> {
  ...
  Future<void> _openAiFillDialog() async {
    final result = await showAiFillDialog(
      context: context,
      sectorLabel: t(context, 'Oto Yıkama / Servis'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Oto Yıkama / Servis Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Oto Yıkama / Servis Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                labelText: t(context, 'Paketler (her satıra bir tane: Ad - Süre - Fiyat)'),
                hintText: t(context, 'İç-Dış Yıkama - 30 dk - 250 TL\nSeramik Kaplama - 1500 TL'),
