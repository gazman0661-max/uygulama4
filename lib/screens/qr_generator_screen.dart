...
/// QR kod üretici. Tamamen yerel/AI'sız bir üretim olduğu için (form
/// akışıyla aynı kategoride) her "Oluştur" işlemi FORM puan havuzundan
/// AppState.costSinglePage (5 puan) düşer — AI havuzunu ETKİLEMEZ.
    ...
    try {
      ...
      if (mounted && saved) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text(t(context, 'QR kod PNG olarak kaydedildi! ✅'))));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text(
                '${isEnglish(context) ? 'Could not save' : 'Kaydedilemedi'}: $e')));
      }
    } finally {
  ...
  Future<void> _share() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text(
                '${isEnglish(context) ? 'Could not share' : 'Paylaşılamadı'}: $e')));
      }
    } finally {
      if (mounted) setState(() => _saving = false);
    }
          ...
          child: Column(
            children: [
              ...
              Align(
                ...
                child: Text(
                  isEnglish(context)
                      ? 'Each QR generation costs ${AppState.costSinglePage} points · Remaining: ${appState.formCredits}'
                      : 'Her QR oluşturma ${AppState.costSinglePage} puan düşer · Kalan: ${appState.formCredits}',
