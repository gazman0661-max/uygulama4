...
class _MainShellState extends State<MainShell> {
  ...
  // 05.09.2026 kaldırıldı (kanka bildirdi: "çift bildirim geliyor, mesaj
  // içeriği bildirimde görünüyor") — burada eskiden Firestore stream'lerini
  // canlı dinleyip yeni bir doküman geldiğinde ANINDA yerel bir bildirim
  // gösteren bir mekanizma (_watchForNewMessages) vardı. Bu, GERÇEK FCM push
  // mimarisi (bkz. push_service.dart, 03.09.2026) kurulmadan ÖNCE, sadece
  // uygulama açıkken bildirim gösterebilmek için eklenmiş bir GEÇİCİ
  // çözümdü. Artık worker her iki olayda da (yeni talep: handleLeadCreate,
  // admin mesajı: handleAdminMailCreate) GERÇEK bir FCM push gönderiyor ve
  // push_service.dart bunu uygulama AÇIKKEN de (_onForegroundMessage) elle
  // gösteriyor — yani bu iki mekanizma AYNI olay için ayrı ayrı birer
  // bildirim gösteriyordu (çift bildirim) ve mailbox tarafında push'un
  // kendisi zaten admin'in girdiği başlık/mesaj metnini OLDUĞU GİBİ
  // taşıdığı için (worker tarafında da düzeltildi, bkz.
  // handleAdminMailCreate) mesaj içeriği bildirim tepsisinde görünüyordu.
  // Tek doğru kaynak artık FCM push — MailboxService/LeadService'in canlı
  // akışları burada SADECE aşağıdaki rozet sayaçları (watchUnclaimedCount /
  // watchUnreadCount) için kullanılıyor, bildirim tetiklemek için değil.
