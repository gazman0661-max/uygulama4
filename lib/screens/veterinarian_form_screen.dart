...
class VeterinarianFormScreen extends StatefulWidget {
  const VeterinarianFormScreen({super.key, this.initialMultiPage = false, this.initialData, this.isEditing = false});
...
class _VeterinarianFormScreenState extends State<VeterinarianFormScreen> {
  ...
  Future<void> _openAiFillDialog() async {
    final result = await showAiFillDialog(
      context: context,
      sectorLabel: t(context, 'Veteriner Kliniği'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Veteriner Kliniği Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Veteriner Kliniği Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
