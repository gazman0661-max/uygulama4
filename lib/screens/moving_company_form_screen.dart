...
/// ve metinler Nakliyat Sitesi için özelleştirildi.
class MovingCompanyFormScreen extends StatefulWidget {
  const MovingCompanyFormScreen({super.key, this.initialData, this.isEditing = false});
  ...
  @override
  State<MovingCompanyFormScreen> createState() => _MovingCompanyFormScreenState();
}

class _MovingCompanyFormScreenState extends State<MovingCompanyFormScreen> {
  ...
  Future<void> _generate() async {
    ...
    final ok = await LocalGenerationHelper.generateSinglePage(
      ...
      kind: ProjectKind.movingCompany,
      formData: formData,
  ...
  Future<void> _openAiFillDialog() async {
    final result = await showAiFillDialog(
      context: context,
      sectorLabel: t(context, 'Nakliyat'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Nakliyat Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Nakliyat Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(labelText: t(context, 'Firma adı'), border: OutlineInputBorder()),
            ),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: t(context, 'Evden Eve Nakliyat - 2500 TL\nOfis Taşıma - 3500 TL'),
