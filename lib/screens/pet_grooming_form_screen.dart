...
class PetGroomingFormScreen extends StatefulWidget {
  const PetGroomingFormScreen({super.key, this.initialData, this.isEditing = false});
...
class _PetGroomingFormScreenState extends State<PetGroomingFormScreen> {
  ...
  Future<void> _openAiFillDialog() async {
    final result = await showAiFillDialog(
      context: context,
      sectorLabel: t(context, 'Pet Kuaförü / Pet Shop'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Pet Kuaförü / Pet Shop Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Pet Kuaförü / Pet Shop Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
