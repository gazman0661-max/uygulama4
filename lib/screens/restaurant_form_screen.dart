...
import '../widgets/confirm_popup.dart'; // 19.09.2026 eklendi
...
import '../widgets/menu_legal_info_field.dart'; // 19.09.2026 eklendi (kanka isteği)
import 'subscription_plans_screen.dart'; // 19.09.2026 eklendi
...
/// bazlı listeleniyor), sadece schemaType 'Restaurant' ve CTA "Rezervasyon Yap"
...
/// çok sayfa seçeneği YOK — restoran/lokanta için tek sayfa yeterli kabul
/// edildi.
...
class _RestaurantFormScreenState extends State<RestaurantFormScreen> {
  ...
  // 19.09.2026 eklendi (kanka isteği) — ürün bazlı yasal bilgi (alerjen/
  // kalori/et türü/alkol-domuz), bkz. widgets/menu_legal_info_field.dart.
  // Abonelikle kilitli (bkz. build() > MenuLegalInfoField.locked ve
  // _generate() > qtMergeMenuLegalInfo çağrısı).
  ...
  List<String>? _sectionOrder; // 15.09.2026 eklendi — Bölüm Sırala/Gizle
  ...
  Map<String, dynamic> _captureFormData() {
    return {
      ...
      'menuLegalInfo': _menuLegalInfo, // 19.09.2026 eklendi
  ...
  void _restoreFromInitialData(Map<String, dynamic> d) {
    ...
    // 19.09.2026 eklendi — abonelik bittikten sonra bile veriyi kaybetmemek
    // için burada HER ZAMAN geri yüklüyoruz; üretime giderken _generate()
    // içinde AKTİF abone değilse boş map ile birleştiriyoruz (veri formda
    // saklı kalır, sadece siteye basılmaz).
  ...
  Future<void> _generate() async {
    ...
    // 19.09.2026 eklendi — ürün bazlı yasal bilgi (alerjen/kalori/et türü/
    // alkol/domuz) SADECE aktif abonelikte siteye basılır (bkz.
    // widgets/menu_legal_info_field.dart > locked). Abonelik yoksa
    // _menuLegalInfo formda saklı kalır ama generator'a hiç ulaşmaz.
    // Kayıtlı veri VARSA ve abonelik yoksa, bilgi sessizce siteden
    // düşmesin diye üretimden önce kullanıcıya sorulur (alerjen bilgisi
    // müşteri güvenliği açısından önemli olabilir).
        ...
        child: ListView(
          ...
          children: [
            ...
            GoogleReviewLinkField(
              ...
              // 20.09.2026 — Google İşletme Profili Kurulum Sihirbazı bilgi kartı
              // (tıklandığı andaki GÜNCEL form değerleri).
              gbp: () => GbpPrefill(
                name: _nameCtrl.text.trim(),
                phone: _phoneCtrl.text.trim(),
                address: _addressCtrl.text.trim(),
