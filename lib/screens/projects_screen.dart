...
class _ProjectsScreenState extends State<ProjectsScreen> {
  ...
  Future<void> _loadLiveStats({bool manual = false}) async {
    if (!mounted) return;
    // 20.09.2026 eklendi (kanka isteği) — sunucuda karşılığı silinmiş (ör.
    // ücretsiz katmanda 6 ay güncellenmediği için kaldırılmış) siteleri
    // yerelde "yayında değil"e çeker; bkz. AppState.reconcilePublishedWithServer.
    // Otomatik açılışta 6 saatte bir, "↻ Yenile"de her seferinde çalışır.
  ...
  Future<void> _unpublishProject(SiteProject project) async {
    final confirmed = await showConfirmPopup(
      ...
      message: '"${project.name}" ' +
          ...
          // 20.09.2026: yayından kaldırmak worker'daki site kaydını ve domain
          // bağlantısını da siler — ücretli domain hakkı geri gelmez.
          (project.isDomainConnected
              ...
              : '') +
          // 20.09.2026: abonelik kota slotu yayından kaldırınca boşalır
          // (bkz. AppState.markProjectUnpublished).
        ...
        while (true) {
          ...
          if (choice == 'buy_domain_once') {
            try {
              ...
              // 20.09.2026 eklendi (kanka isteği): domain satın almasının
              // bir parçası olarak +1 ek site yayın hakkı (bkz.
              // domain_purchase_sheet.dart > _purchase — AYNI ürün, AYNI kural).
              try {
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return _ProjectCard(
                          ...
                          // 20.09.2026 — Google Search Console doğrulama kodu (yayındaki projelerde).
          ...
          child: Column(
            ...
            children: [
              ...
              if (onTransfer != null || onSearchConsole != null) ...[
                ...
                Row(
                  children: [
                    // 20.09.2026 eklendi (kanka isteği) — Search Console bağlantısı;
                    // kod kayıtlıysa yeşil + tik. Kilit/uyarılar sheet'in içinde
                    // (bkz. widgets/search_console_sheet.dart).
              ...
              // gösteriliyor.
              //
              // 18.09.2026 eklendi (kanka isteği — "ücretsiz sitelere 6 aylık
              // sayaç koyalım") — üçüncü bir satır: proje YAYINDA ama
              // isPremium DEĞİLSE (yani ne domain bağlı ne mini paket/abonelik
              // aktif — bkz. SiteProject.isPremium), 6 aylık ücretsiz yayın
              // süresinden kalan zaman gösterilir (bkz.
              // SiteProject.freeTierPublishExpiresAt dokümanı). isPremium true
              // olan sitelerde bu satır hiç görünmez — domain/mini paket/
              // abonelik satırlarıyla karışmasın diye.
