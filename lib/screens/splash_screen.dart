...
class _SplashScreenState extends State<SplashScreen>
  ...
  void initState() {
    ...
    // 18.09.2026 eklendi — bkz. AppUpdateService dokümanı: worker'a
    // splash animasyonuyla PARALEL sorulur, ekstra bekleme süresi
    // EKLEMEZ (animasyon zaten en az birkaç yüz ms sürüyor).
    final updateCheckFuture = AppUpdateService.check(isEnglish: isEnglish(context));

    // Toplam 3.5 saniye splash gösterimi.
    Future.delayed(const Duration(milliseconds: 3500), () async {
      if (!mounted) return;
      final updateResult = await updateCheckFuture;
      if (!mounted) return;

      if (updateResult.severity == UpdateSeverity.mandatory) {
        // ZORUNLU güncelleme: MainShell'e HİÇ geçilmez — geri tuşu da
        // engellenir (PopScope canPop:false), kullanıcının TEK seçeneği
        // Play Store'a gitmektir.
        await showDialog<void>(
          ...
          builder: (dialogContext) => PopScope(
            canPop: false,
            child: AlertDialog(
              title: Text(isEnglish(dialogContext) ? 'Update required' : 'Güncelleme gerekli'),
              content: Text(updateResult.message),
              actions: [
                TextButton(
                  onPressed: () => AppUpdateService.openStore(updateResult.playStoreUrl),
                  child: Text(isEnglish(dialogContext) ? 'Update now' : 'Şimdi güncelle'),
                ),
              ],
            ),
          ),
        );
        // Diyalog kapatılamayacağı için buraya normal şartlarda hiç
        // gelinmez — yine de MainShell'e geçmeyi ENGELLEMEK için
        // fonksiyonu burada bitiriyoruz.
        return;
      }

      if (updateResult.severity == UpdateSeverity.optional && mounted) {
        await showDialog<void>(
          context: context,
          builder: (dialogContext) => AlertDialog(
            title: Text(isEnglish(dialogContext) ? 'New version available' : 'Yeni sürüm mevcut'),
            content: Text(updateResult.message),
            actions: [
              TextButton(
                onPressed: () => Navigator.of(dialogContext).pop(),
                child: Text(isEnglish(dialogContext) ? 'Later' : 'Daha sonra'),
              ),
              TextButton(
                onPressed: () {
                  Navigator.of(dialogContext).pop();
                  AppUpdateService.openStore(updateResult.playStoreUrl);
                },
                child: Text(isEnglish(dialogContext) ? 'Update' : 'Güncelle'),
              ),
            ],
          ),
        );
      ...
      Navigator.of(context).pushReplacement(
        PageRouteBuilder(
          ...
          // 01.09.2026 değiştirildi — artık doğrudan HomeScreen değil,
          // Ana Sayfa/Projeler sekmeleri ve alt sabit gezinme çubuğunu
          // barındıran MainShell açılıyor (bkz. main_shell.dart).
                        ...
                        child: Transform.translate(
                          ...
                          child: Column(
                            children: [
                              ...
                              Text(
                                t(context, 'WEB SİTESİ OLUŞTURUCU'),
                                style: TextStyle(
