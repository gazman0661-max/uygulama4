...
class MakeupArtistFormScreen extends StatefulWidget {
  const MakeupArtistFormScreen({super.key, this.initialMultiPage = false, this.initialData, this.isEditing = false});
...
class _MakeupArtistFormScreenState extends State<MakeupArtistFormScreen> {
  ...
  Future<void> _openAiFillDialog() async {
    final result = await showAiFillDialog(
      context: context,
      sectorLabel: t(context, 'Makyaj Sanatçısı'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Makyaj Sanatçısı Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Makyaj Sanatçısı Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            GalleryPickerField(
              label: t(context, 'Çalışma Örnekleri'),
