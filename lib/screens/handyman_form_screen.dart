...
class HandymanFormScreen extends StatefulWidget {
  const HandymanFormScreen({super.key, this.initialData, this.isEditing = false});
...
class _HandymanFormScreenState extends State<HandymanFormScreen> {
  ...
  Future<void> _openAiFillDialog() async {
    final result = await showAiFillDialog(
      context: context,
      sectorLabel: t(context, 'Usta Hizmetleri'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Usta Hizmetleri Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Usta Hizmetleri Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
