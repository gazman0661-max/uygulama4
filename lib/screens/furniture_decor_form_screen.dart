...
/// ve metinler Mobilyacı / Dekorasyon için özelleştirildi.
class FurnitureDecorFormScreen extends StatefulWidget {
  const FurnitureDecorFormScreen({super.key, this.initialData, this.isEditing = false});
  ...
  @override
  State<FurnitureDecorFormScreen> createState() => _FurnitureDecorFormScreenState();
}

class _FurnitureDecorFormScreenState extends State<FurnitureDecorFormScreen> {
  ...
  Future<void> _generate() async {
    ...
    final ok = await LocalGenerationHelper.generateSinglePage(
      ...
      kind: ProjectKind.furnitureDecor,
      formData: formData,
  ...
  Future<void> _openAiFillDialog() async {
    final result = await showAiFillDialog(
      context: context,
      sectorLabel: t(context, 'Mobilyacı / Dekorasyon'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Mobilyacı / Dekorasyon Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Mobilyacı / Dekorasyon Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                labelText: t(context, 'Ürün Kategorileri (her satıra bir tane: Ad - Açıklama - Fiyat Aralığı)'),
                hintText: t(context, 'Oturma Grubu - Modern Tasarım - 15000 TL\'den başlayan\nYatak Odası Takımı - Ahşap - 25000 TL\'den başlayan'),
