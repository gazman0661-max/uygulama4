...
class KuaforFormScreen extends StatefulWidget {
  const KuaforFormScreen({super.key, this.initialData, this.isEditing = false});
...
class _KuaforFormScreenState extends State<KuaforFormScreen> {
  ...
  /// 30.08.2026 eklendi — "✨ AI ile Doldur" popup'ını açar (bkz.
  /// widgets/ai_fill_dialog.dart). Dönen Map'teki alanları ilgili
  /// controller'lara yazar. AI HİÇBİR ZAMAN akışı tıkamaz: Worker henüz
  /// bağlanmadıysa (AiFillService.isConfigured=false) veya istek
  /// başarısız olursa dialog yine de adres/telefon/sosyal medya gibi
  /// yerel alanları uygular, kullanıcı formu istediği an elle de
  /// doldurabilir/düzenleyebilir.
  Future<void> _openAiFillDialog() async {
    ...
    setState(() {
      ...
      if (result['address'] != null) _addressCtrl.text = result['address']!;
      if (result['phone'] != null) _phoneCtrl.text = result['phone']!;
      if (result['whatsapp'] != null) _whatsappCtrl.text = result['whatsapp']!;
      if (result['instagram'] != null) _instagramCtrl.text = result['instagram']!;
      // AI'dan gelen yaratıcı alanlar — Worker henüz bağlanmadıysa bu
      // anahtarlar hiç gelmez, formun geri kalanı etkilenmez.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isEditing
              ? '${t(context, 'Kuaför / Berber Sitesi')} • ${t(context, 'Düzenle')}'
              : t(context, 'Kuaför / Berber Sitesi'),
        ),
      ),
      body: SafeArea(
        child: ListView(
          ...
          children: [
            // 30.08.2026 eklendi — "✨ AI ile Doldur". Mevcut PillButton
            // deseniyle (bkz. widgets/pill_button.dart) aynı görsel dilde,
            // formun en üstünde: kullanıcı isterse formu AI'ye
            // doldurtabilir, istemezse aşağıdaki alanları hiç dokunmadan
            // elle doldurmaya devam eder — buton tamamen opsiyonel.
            Align(
