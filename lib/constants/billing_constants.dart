...
///
/// 20.09.2026 EKLENDİ (kanka isteği) — Bu ürün artık +1 EK SİTE YAYIN HAKKI
/// da içerir: gerçek bir ödeme tamamlandığında (ilk bağlama, yenileme ve
/// site devrinde "yıllık domain öde" yolunun HEPSİ) AppState.
/// addPurchasedPublishCredit() çağrılır (bkz. domain_purchase_sheet.dart >
/// _purchase, projects_screen.dart > buy_domain_once). HEDİYE hakkıyla
/// (redeemGiftDomainConnect) ya da abonelik domain kotasıyla bağlayan bu
/// hakkı ALMAZ — sadece ödeme yapan alır. Sadece ilk satın almada verilmesi
/// istenirse domain_purchase_sheet.dart'ta `if (!widget.isRenewal)` ekle.
/// Play Console'daki ürün açıklaması da buna göre güncellenmeli.
///
/// 20.09.2026 EKLENDİ (kanka isteği) — PREMİUM KİLİTLERİNE (Talep Kutusu, tam
/// galeri, harita, talep formu, Google yorum butonu, ziyaretçi sayısı) bir
/// yenisi eklendi: "Google İşletme Profili Kurulum Sihirbazı" (bkz.
/// widgets/gbp_setup_wizard.dart). Kilit kuralı diğerleriyle AYNI ve TEK: sadece
/// ÜCRETSİZ katmanda kilitli (SiteProject.isPremium == false). Bu ürün (domain),
/// abonelik paketleri ve eski mini paket sihirbazı AÇAR. Ürün konumlandırması:
/// "Profili biz açmıyoruz, entegrasyonu kolaylaştırıyoruz" — Google profilini
/// Google, kullanıcının hesabıyla açar; sihirbaz sadece bilgi kartı hazırlar,
/// Google'ın ücretsiz kayıt ekranına yönlendirir ve yorum linkini siteye bağlar.
/// Bu yüzden Play Console/mağaza metinlerinde "Google profili açıyoruz/kuruyoruz"
/// gibi ifadeler KULLANILMAMALI (bkz. DEGISIKLIKLER_20_09_2026_GOOGLE_ISLETME_SIHIRBAZI.md).
...
///     formu, Google yorum butonu + Google İşletme Profili Kurulum Sihirbazı,
///     ziyaretçi sayısı — bkz. SiteProject.isPremium)
