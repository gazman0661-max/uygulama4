...
class AppState extends ChangeNotifier {
  ...
  // premium olamaz.
  ///
  /// 23.09.2026 GÜNCELLENDİ (kanka bulgusu — "yeni site oluşturunca abonelik
  /// olmasına rağmen alanlar kilitli görünüyor, ancak yayınlayıp Düzenle'ye
  /// tekrar girince açılıyor" akışı). Kök sebep: SiteProject.isPremium →
  /// isSubscriptionQuotaActive → subscriptionQuotaExpiresAt SADECE
  /// assignProjectToSubscriptionQuota çağrıldığında (yani grantPublishRight
  /// İÇİNDEN, "Yayınla" dendiği AN) doluyor — kota bir projeye "oluşturulurken"
  /// değil "yayınlanırken" atanıyor. Bu yüzden henüz yayınlanmamış / henüz
  /// kaydedilmemiş yepyeni bir proje kendi isPremium'ında hep false görünüyor,
  /// hesabın boş kotası olsa bile.
  ///
  /// Çözüm: burada projenin kendi isPremium'ına ek olarak, "proje henüz kota
  /// almamış olsa bile hesabın aktif aboneliği VE boş bir kota slotu var mı"
  /// diye canPublishProject/assignProjectToSubscriptionQuota ile AYNI desende
  /// (activeSubscriptionTier + subscriptionQuotaUsed < tier.siteQuota) BAKILIR.
  /// Bu SADECE ekrandaki kilit kararı için anlık bir kontroldür — hiçbir
  /// projeye KALICI olarak yazılmaz (subscriptionQuotaExpiresAt burada
  /// SET EDİLMEZ). Gerçek slot rezervasyonu (harcama) hâlâ SADECE
  /// grantPublishRight → assignProjectToSubscriptionQuota içinde, kullanıcı
  /// gerçekten "Yayınla"ya bastığında olur — böylece kullanıcı formu tek
  /// seferde doldurup tek seferde yayınlayabilir, ikinci bir Düzenle turuna
  /// gerek kalmaz.
