...
class AppState extends ChangeNotifier {
  ...
  Future<void> _onAuthChanged(User? user) async {
    ...
    try {
      ...
      // kesilmez. 05.09.2026 eklendi: hata artık loglanıyor (bkz.
      // _syncProjectToCloudIfSignedIn'deki aynı gerekçe).
      DebugLogService.instance
          .log('Giriş sonrası bulut senkronu başarısız: $e');
      debugPrint('$st');
    }
  }
  ...
  Future<bool> _syncProjectToCloudIfSignedIn(SiteProject project) async {
    ...
    try {
      ...
      // eklemeye gerek yok. 05.09.2026 eklendi: hata artık sessizce
      // yutulmuyor, `debugPrint` ile loglanıyor ki "senkron neden hep
      // başarısız oluyor" (rules, network, veri formatı vb.) `adb logcat`
      // veya debug konsolundan görülebilsin.
      DebugLogService.instance
          .log('Proje buluta yazılamadı (id=${project.id}): $e');
      debugPrint('$st'); // stack trace sadece adb için, ekranda göstermiyoruz (uzun)
      return false;
    }
  }
