...
class AppState extends ChangeNotifier {
  ...
  Future<bool> assignDomainQuota(String projectId) async {
    ...
    if (uid != null) {
      unawaited(SubscriptionQuotaSyncService.assignDomain(siteId: projectId, uid: uid).catchError((_) {}));
    }
    return true;
  }
  ...
  Future<void> unassignDomainQuota(String projectId, {bool alsoDisconnect = true}) async {
    ...
    await _persistProjects();
    unawaited(_syncProjectToCloudIfSignedIn(projects[idx]));
    unawaited(SubscriptionQuotaSyncService.unassignDomain(siteId: projectId).catchError((_) {}));

    if (alsoDisconnect) {
      try {
        await DomainService.disconnect(siteId: projectId);
      } catch (_) {
  ...
  Future<bool> assignProjectToSubscriptionQuota(String projectId) async {
    ...
    if (uid != null) {
      unawaited(
        SubscriptionQuotaSyncService.assign(siteId: projectId, uid: uid).catchError((_) {}),
      );
    }
    return true;
  }
  ...
  Future<void> unassignProjectFromSubscriptionQuota(String projectId) async {
    ...
    await _persistProjects();
    unawaited(_syncProjectToCloudIfSignedIn(projects[idx]));
    unawaited(SubscriptionQuotaSyncService.unassign(siteId: projectId).catchError((_) {}));
  }
  ...
  Future<TransferClaimOutcome> claimTransferredProject(
    ...
    if (result.quotaOutcome != 'kept_via_subscription') {
      unawaited(SubscriptionQuotaSyncService.unassign(siteId: claimed.id).catchError((_) {}));
    }
