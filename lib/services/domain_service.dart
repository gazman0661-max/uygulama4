...
class DomainService {
  ...
  static Future<DomainConnectResult> connect({
    ...
    // GÜVENLİK (18.09.2026 eklendi): worker artık bu uçta owner_token
    // kontrolü yapıyor (bkz. index.mjs > forbiddenIfOwnerMismatch) —
    // header eklenmezse kendi sitende bile 403 alırsın. SiteProject.ownerToken
    // publish sırasında worker'dan dönüp kaydedilen değerdir.
    String? ownerToken,
  }) async {
    ...
    try {
      res = await http.post(
        ...
        headers: {
          ...
          // 19.09.2026: sıkı modda worker doğrulanmış bir domain satın alımı
          // (ya da abonelik domain slotu) arar — bkz. worker > handleDomainConnect.
  ...
  static Future<DomainRenewResult> renew({required String siteId, String? ownerToken}) async {
    ...
    try {
      res = await http.post(
        ...
        headers: {
          if (ownerToken != null) 'x-owner-token': ownerToken,
          ...await authHeaderIfSignedIn(), // 19.09.2026 — bkz. connect()
        },
      );
    } catch (e) {
      throw DomainException('İnternet bağlantısı sorunu: $e');
  ...
  static Future<void> disconnect({required String siteId, String? ownerToken}) async {
    ...
    try {
      res = await http.delete(
        ...
        headers: {
          if (ownerToken != null) 'x-owner-token': ownerToken,
          // GÜVENLİK (20.09.2026 eklendi — bulgu #8 fix'inin istemci tarafı):
          // worker artık owner_token'ı OLMAYAN (eski) siteler için bu uçta
          // sıkı modda giriş yapmış olmayı VE bilinen owner_uid ile
          // eşleşmeyi şart koşuyor (bkz. index.mjs > forbiddenIfLegacySite
          // Unauthorized). Bu header eklenmezse, giriş yapmış olsan bile
          // eski bir siteni artık domain'den ayıramazsın (401 alırsın).
          ...await authHeaderIfSignedIn(),
        },
      );
    } catch (e) {
