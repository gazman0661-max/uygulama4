...
class DownloadService {
  ...
  static Future<File> saveMultiPageZip({
    ...
    await Share.shareXFiles(
      ...
      text: 'Sitora AI ile üretildi 🚀 — zip\'i açıp istediğin hosting\'e yükleyebilirsin.',
    );
    return zipFile;
  }
  ...
  /// QR Kod gibi PNG çıktıları için: kullanıcının kendi seçtiği konuma
  /// kaydeder. Döner: kaydedildiyse true, iptal edildiyse false.
  ...
  /// PNG'yi doğrudan paylaşım menüsünden (WhatsApp, Instagram vb.) paylaşır.
  static Future<void> sharePng({
    required Uint8List pngBytes,
    String fileName = 'qr-kod.png',
    String text = 'Sitora AI ile üretildi 🚀',
  }) async {
    final dir = await getTemporaryDirectory();
    final file = File('${dir.path}/$fileName');
