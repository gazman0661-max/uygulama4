...
class PushService {
  ...
  // GÜVENLİK (18.09.2026 eklendi): worker artık /api/fcm-token DELETE için de
  // Authorization: Bearer <idToken> istiyor (bkz. handleFcmTokenUnregister).
  // Çıkış anında (authStateChanges -> null tetiklendiğinde) currentUser ZATEN
  // null olmuş oluyor, o an YENİ bir idToken alınamaz — bu yüzden EN SON
  // BAŞARILI kayıt sırasında alınmış idToken'ı burada saklayıp çıkışta onu
  // kullanıyoruz. Süresi dolmuşsa (uzun oturum) worker 401 döner, silme
  // best-effort olduğu için sorun değil — kayıt zaten worker'da geçersiz
  // push denemesinde otomatik temizlenir (bkz. sendFcmToToken).
  ...
  Future<void> init() async {
    ...
    try {
      ...
      // 20.09.2026: izin sonucu analitiğe (sadece değişince loglanır).
      ...
      // 20.09.2026: kullanıcı arka plandaki/kapalı bir push'a dokunup
      // uygulamayı açtığında (push_opened) — Talep/Posta bildirimlerinin
      // kullanıcıyı geri getirip getirmediğini ölçmek için.
  ...
  Future<void> _registerToken(String uid, String token) async {
    ...
    try {
      // GÜVENLİK (18.09.2026 düzeltildi): worker artık body.uid'e
      // güvenmiyor (bkz. index.mjs > handleFcmTokenRegister dokümanı) —
      // isteğin `Authorization: Bearer <Firebase ID Token>` taşıması
      // ZORUNLU, uid token'ın kendisinden okunuyor. Token alınamazsa
      // (nadiren, ör. anlık ağ sorunu) sessizce vazgeçilir — bir sonraki
      // token yenilenmesinde/girişte tekrar denenir.
  ...
  Future<void> _unregisterLastToken() async {
    ...
    // idToken yoksa (ör. uygulama az önce açıldı, hiç register çağrılmadan
    // direkt çıkış yapıldı) worker artık AUTH ZORUNLU tuttuğu için bu
    // isteği hiç atmıyoruz — atsak zaten 401 alırdık. Best-effort: worker
    // eninde sonunda geçersiz push denemesinde kaydı kendi temizler.
    if (idToken == null) return;
    try {
      await http
          .delete(
