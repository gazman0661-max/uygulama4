...
class LocalGenerationHelper {
  ...
  /// Oluştur'a basarsa üretim İPTAL edilir (proje KAYDEDİLMEZ) ve
  /// bilgilendirici kilit popup'ı gösterilir — kullanıcı ya ücretsiz bir
  /// seçeneğe döner ya da (ileride) premium'a geçer.
  // 18.09.2026 eklendi (kanka isteği — "Serbest Font Seçimi") —
  // [selectedFontPackageId] artık da kontrol ediliyor: bkz.
  // [premiumFontPackageIds]/[isPremiumFontPackage] dokümanı. AYNI desen —
  // TypographyPickerField seçim aşamasında hiçbir şeyi engellemez,
  // kısıtlama SADECE burada, "Oluştur"a basılma anında uygulanır.
  ...
  static Future<bool> generateSinglePage({
    ...
    String? selectedThemeId,
    String? selectedLayoutStyle,
    // 18.09.2026 eklendi — bkz. [_isPremiumChoiceLocked] dokümanı ("Serbest
    // Font Seçimi"). Formun o an seçili tipografi paketi id'sini geçirmek
    // yeterli; 'custom_font' ise ve hesap premium değilse aynı gate devreye
    // girer.
    String? selectedFontPackageId,
  }) async {
    final appState = context.read<AppState>();

    // 02.09.2026 düzeltildi — ÖNEMLİ DONMA DÜZELTMESİ: detachQtSlotForNewProject
    ...
    try {
      // 06.09.2026 eklendi — bkz. [_isPremiumChoiceLocked] dokümanı,
      // FreePlanPageLimitService gate'iyle AYNI sıralama mantığı: kilitli
      // bir seçimde gereksiz yere ileri gidilmesin diye en başta kontrol
      // edilir.
  ...
  static Future<bool> generateMultiPage({
    ...
    String? selectedThemeId,
    String? selectedLayoutStyle,
    // 18.09.2026 eklendi — bkz. generateSinglePage'teki AYNI parametre.
    String? selectedFontPackageId,
    ...
    // düzeltmesi: detachQtSlotForNewProject artık
    ...
    try {
      ...
      // buradaki sıralama da çok sayfa limitinden ÖNCE — kilitli seçimde
      // sayfa limiti hiç hesaplanmaz.
      if (_isPremiumChoiceLocked(
        isPremiumAccount: appState.qtCurrentIsPremium,
        selectedThemeId: selectedThemeId,
        selectedLayoutStyle: selectedLayoutStyle,
      ...
      // BURADA reddedilir — henüz updateQtGeneratedFiles ÇAĞRILMADIĞI için
      // proje kaydedilmez/üzerine yazılmaz (varsa ÖNCEKİ kayıtlı hâli
      // olduğu gibi kalır).
