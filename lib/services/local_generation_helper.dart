...
class LocalGenerationHelper {
  static Future<bool> generateSinglePage({
    ...
    // 26.08.2026 güncellendi — Kullanıcı BOŞ YERE beklemesin diye reklam
    // artık üretim BİTTİKTEN SONRA değil, "Siteyi Oluştur"a tıklanır
    // tıklanmaz (kota geçtiği an) AÇILIYOR — await EDİLMEDEN (unawaited),
    // ki aşağıdaki senkron/local üretim (buildHtml) reklamla AYNI ANDA
    // arka planda çalışsın. Kullanıcı reklamı izlerken site zaten hazır
    // oluyor; reklam kapandığında ekstra bir bekleme yaşanmıyor. Sadece
    // YENİ site oluşturmada (düzenleme akışında DEĞİL).
    if (!isEditing) {
      unawaited(AdsService.instance.showInterstitial(placement: 'site_create'));
    }
  ...
  static Future<bool> generateMultiPage({
    ...
    // bkz. generateSinglePage'teki AYNI YORUM — reklam üretimle PARALEL
    // açılır, kullanıcı reklamı izlerken site zaten arka planda oluşuyor.
    if (!isEditing) {
      unawaited(AdsService.instance.showInterstitial(placement: 'site_create'));
    }
