...
/// AI çağrısı, kota kontrolü, WorkerService YOKTUR —
...
class LocalGenerationHelper {
  static Future<bool> generateSinglePage({
    ...
    // AI havuzunu ETKİLEMEZ, tamamen ayrı/bağımsız bir sayaç.
  ...
  static Future<bool> generateMultiPage({
    ...
    // AI havuzunu ETKİLEMEZ, tamamen ayrı/bağımsız bir sayaç.
    if (!await appState.ensureFormQuotaFor(AppState.costMultiPage)) {
      if (context.mounted) await showQuotaLimitPopup(context);
      return false;
    }
    ...
    try {
      ...
      await appState.consumeFormQuota(AppState.costMultiPage);
      unawaited(AnalyticsService.logSiteGenerated(
        source: 'form',
        mode: 'multi',
        kind: kind.name,
