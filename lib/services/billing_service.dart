...
/// (hangi proje, hangi puan paketi) göre yorumlayıp asıl işlemi
/// (removeWatermarkForProject / grantPublishRight / addPurchasedPoints)
...
///
/// SUNUCU TARAFI MAKBUZ DOĞRULAMASI — TODO:
/// Şu an satın alma, Play Store'un istemci tarafına döndürdüğü
/// `PurchaseDetails.status == purchased` bilgisine güvenerek "başarılı"
/// sayılıyor. Bu, sahte/hacklenmiş APK'lerle istismar edilebilir (cihazda
/// billing response'u taklit etme). Gerçek lansman öncesi:
///   1) `purchaseDetails.verificationData.serverVerificationData` (imzalı
///      makbuz) worker'a POST edilmeli,
///   2) worker Google Play Developer API (Purchases.products.get /
///      Purchases.subscriptions.get) ile makbuzu Google'a doğrulatmalı,
///   3) SADECE doğrulama başarılı dönünce AppState tarafı işlemi uygular.
/// Bu dosyadaki `_verifyPurchaseServerSide` metodu bu adımın YER TUTUCUSU —
/// şimdilik hep `true` döner.
class BillingService {
  ...
  /// TODO (bkz. dosya başı açıklaması): şimdilik yer tutucu — gerçek
  /// lansmandan önce worker üzerinden Google Play Developer API'ye
  /// makbuz doğrulaması eklenmeli. `purchase.verificationData
  /// .serverVerificationData` worker'a POST edilecek alan budur.
