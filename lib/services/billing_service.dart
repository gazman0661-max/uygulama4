...
class BillingService {
        ...
        case PurchaseStatus.restored:
          try {
            ...
            if (verified) {
              ...
              // 20.09.2026 DÜZELTME: `restored` (uygulama açılışındaki
              // restorePurchases / yeni cihaz) DA bu dala düşüyordu ve her
              // seferinde 'purchase' olayı loglanıp Firebase gelir raporunu
              // şişiriyordu (özellikle aktif abonesi olan kullanıcıda her
              // açılışta). Artık sadece GERÇEK yeni satın alma loglanır.
  ...
  Future<bool> _verifyPurchaseServerSide(PurchaseDetails purchase) async {
    // GÜVENLİK (19.09.2026 değişti): worker artık uid'i body'den değil,
    // `Authorization: Bearer <Firebase ID Token>` header'ından doğrulayıp
    // okuyor (bkz. worker > handleVerifyPurchase). Giriş yoksa istek
    // atılmaz — GÜVENLİ TARAF: reddet (makbuz tüketilmez/tamamlanmaz, Play
    // bir sonraki açılışta purchaseStream'e tekrar koyar).
  ...
  Future<bool> _verifySubscriptionServerSide(PurchaseDetails purchase) async {
    ...
    try {
      // GÜVENLİK (19.09.2026 değişti): uid artık body'de gönderilmiyor —
      // worker onu doğrulanmış ID token'ından okuyor (bkz. worker >
