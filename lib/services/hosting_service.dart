...
class HostingService {
  ...
  /// [ownerUid] OPSİYONEL: verilirse hem D1'deki `sites.owner_uid` alanına
  /// yazılır (06.09.2026 eklendi — destek/moderasyon "bu hesabın tüm
  /// siteleri" diye net sorgulayabilsin diye, bkz. schema.sql), hem de
  /// üretilen HER .html sayfaya "Gelen Talepler" (lead) formunun ihtiyaç
  /// duyduğu `window.__SITORA_LEAD__` konfigürasyonu enjekte edilir (bkz.
  /// [_injectLeadConfig] ve templates/html/shared_html_blocks.dart >
  /// leadFormMarkup). Boş bırakılırsa site yine yayınlanır, sadece iletişim
  /// formundaki "Talep Gönder" gönderilemez hatası verir (telefon/WhatsApp
  /// butonları etkilenmez), D1'de owner_uid de NULL kalır. Çağıran taraf
  /// (widgets/publish_sheet.dart) bunu AuthService.instance.currentUser?.uid
  /// ile dolduruyor — yayınlama zaten giriş gerektirdiği için (bkz.
  /// login_gate.dart) bu değer PRATİKTE her zaman mevcuttur.
  static Future<PublishResult> publish({
    ...
    String? ownerToken,
    // 06.09.2026 eklendi — worker'daki D1'e site dilini kaydeder (bkz.
    // sites.site_lang), premiumDowngradeSweep süresi dolan siteye rozeti
    // geri enjekte ederken doğru dilde (TR/EN) yazsın diye. 'tr'/'en' dışı
    // bir değer worker tarafında yok sayılır.
    String? lang,
    // 15.09.2026 eklendi (kanka isteği) — "Talepler nereye gelsin?" seçimi.
    // 'box' (varsayılan): sadece uygulama içi Talep Kutusu (mevcut davranış,
    // değişiklik yok). 'email': ziyaretçinin kendi mail istemcisinde bir
    // mailto: linki açılır, worker'a HİÇ istek atılmaz. 'both': ikisi de.
    // BİLİNÇLİ OLARAK Resend/başka bir ücretli sunucu-taraflı mail API'si
    // KULLANILMIYOR — mailto tamamen istemci tarafında, ücretsiz ve
    // sınırsızdır (bkz. shared_html_blocks.dart > leadFormMarkup). Bu ayar
    // worker'a/D1'e HİÇ gönderilmez — sadece yayınlanan HTML'e gömülür,
    // çünkü mailto'nun çalışması için sunucu tarafında bilinmesine gerek
    // yoktur.
    String leadDelivery = 'box',
    // 'email'/'both' seçildiğinde taleplerin gideceği adres. Boş bırakılırsa
    // (beklenmedik durum) leadFormMarkup JS'i sessizce 'box' davranışına
    // düşer (bkz. o dosyadaki openMailto).
    String? leadEmail,
  }) async {
    ...
    try {
      res = await http.post(
        ...
        // 19.09.2026: worker sıkı modda Firebase ID token ister ve ownerUid'i
        // ARTIK bu token'dan okur (body'deki ownerUid yalnızca eski/kapalı mod
        // uyumluluğu için gönderiliyor).
    ...
    // 20.09.2026: her başarısızlık nedeni analitiğe düşer (publish_failed).
    ...
    // 20.09.2026: ownerToken yoksa bu proje için worker daha önce token
    // vermemiştir = ilk yayın (güncellemelerden ayrılsın diye).
  ...
  static Future<void> unpublish({required String siteId, String? ownerToken}) async {
    ...
    final res = await http.delete(
      ...
      headers: {
        ...
        // 19.09.2026: owner_token'ı olmayan ESKİ siteler için worker (sıkı mod)
        // uid ile sahiplik doğrular.
  ...
  /// 15.09.2026 eklendi — [leadDelivery] ('box'/'email'/'both') ve
  /// [leadEmail] de bu config'e gömülür (bkz. [publish] dokümanı). Worker'a
  /// hiç uğramaz; leadFormMarkup'taki JS bunu doğrudan okur.
