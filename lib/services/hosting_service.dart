...
class PublishResult {
  ...
  final String url;
  PublishResult({required this.siteId, required this.subdomain, required this.url});
...
/// GET /api/sites/:id/stats yanıtı — bkz. HostingService.fetchStats.
class SiteStats {
  final String siteId;
...
class HostingService {
  ...
  static Future<void> unpublish({required String siteId}) async {
    if (!HostingConfig.isConfigured) throw HostingNotConfiguredException();
    final res = await http.delete(Uri.parse('${HostingConfig.baseUrl}/api/sites/$siteId'));
    if (res.statusCode < 200 || res.statusCode >= 300) {
  ...
  static String _injectReportWidget({required String html, required String siteId}) {
    final widget = reportWidgetSnippet(siteId: siteId);
...
''';
  ...
  static String reportWidgetSnippet({required String siteId}) {
    ...
    return '''
<div id="sitora-report-widget" style="position:fixed;bottom:14px;right:14px;z-index:999999;font-family:sans-serif;">
  <button id="sitora-report-btn" style="background:#B91C1C;color:#fff;border:none;border-radius:999px;padding:8px 14px;font-size:12px;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,.25);opacity:.85;">🚩 Şikayet Et</button>
  <div id="sitora-report-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);align-items:center;justify-content:center;">
    <div style="background:#141821;color:#fff;border-radius:14px;padding:18px;max-width:320px;width:90%;">
      <div style="font-weight:bold;margin-bottom:8px;">Bu siteyi bildir</div>
      <select id="sitora-report-reason" style="width:100%;padding:8px;margin-bottom:8px;border-radius:8px;">
        <option value="offensive">Rahatsız edici / uygunsuz içerik</option>
        <option value="illegal">Yasa dışı içerik</option>
        <option value="hate">Nefret söylemi / taciz</option>
        <option value="other">Diğer</option>
      </select>
      <textarea id="sitora-report-details" placeholder="Kısaca açıkla (zorunlu)" style="width:100%;min-height:60px;padding:8px;border-radius:8px;margin-bottom:8px;"></textarea>
      <div style="display:flex;gap:8px;">
        <button id="sitora-report-cancel" style="flex:1;padding:10px;border-radius:8px;border:1px solid #444;background:transparent;color:#fff;">İptal</button>
        <button id="sitora-report-send" style="flex:1;padding:10px;border-radius:8px;border:none;background:#B91C1C;color:#fff;">Gönder</button>
      </div>
    </div>
  </div>
</div>
...
(function(){
  ...
  send.onclick = function(){
    ...
    if (!details) { alert('Lütfen kısa bir açıklama yaz.'); return; }
    send.disabled = true;
    fetch(endpoint, {
      method: 'POST',
      ...
      alert('Bildirimin ulaştı, teşekkürler. En kısa sürede incelenecek.');
      modal.style.display = 'none';
      send.disabled = false;
    }).catch(function(){
      alert('Bildirim gönderilemedi, internetini kontrol edip tekrar dener misin?');
      send.disabled = false;
    });
  };
})();
