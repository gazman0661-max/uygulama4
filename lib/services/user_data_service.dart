...
///                          hakkı — satın alınandan AYRI tutulur ki Ayarlar ekranında
///                          ikisi ayrı ayrı gösterilebilsin, bkz. settings_sheet.dart)
...
///   giftDomainConnectCredits       : int (05.09.2026 eklendi — aynı mantık, "özel
///                          domain bağlama hakkı" hediyesi bakiyesi; harcandığında
///                          SADECE ödeme atlanır, asıl bağlanma isteğini kullanıcı
///                          domain_connect_screen.dart'tan normal akışla atar, bkz.
///                          AppState.redeemGiftDomainConnect)
///   giftMiniPackageCredits         : int (aynı mantık — "1 aylık mini paket hakkı"
///                          hediyesi bakiyesi; tamamen yerel olduğu için harcandığında
///                          doğrudan AppState.activateMiniPackage'i de tetikler, bkz.
///                          AppState.redeemGiftMiniPackage)
///   createdAt            : timestamp
///   updatedAt            : timestamp
///
...
///   }
///
/// ÖNEMLİ — GÜVENLİ VERİ TAŞIMA KARARI:
/// Bir misafir kullanıcı bir hesaba İLK KEZ giriş yaptığında (yani bu uid'e
/// ait Firestore dokümanı henüz hiç yoksa), bulut hesabı SIFIRDAN DOLU hak
/// ile değil, cihazda O AN KALAN hak/proje neyse onunla oluşturulur
/// (bkz. [fetchOrCreateUserDoc]). Bu, "misafirken hakkı bitir, sonra giriş
/// yap, bedava tekrar dolu hak kap" istismarını engeller. Doküman zaten
...
class UserDataService {
  ...
  Future<Map<String, dynamic>> fetchOrCreateUserDoc({
    ...
    // İLK GİRİŞ: güvenli taşıma — cihazda kalan hak/proje neyse
    // bulutta da o kadarla başlanır (dolu hakla/boş projeyle değil).
  ...
  /// Hesap genelindeki bakiyeleri (ücretsiz/satın alınan yayın hakkı) tek
  /// seferde bulutla eşitler. AppState içinde bu değerlerden herhangi biri
  /// değiştiğinde (yayın hakkı satın alma/harcama), kullanıcı giriş
  /// yapmışsa çağrılır — bkz. AppState._syncAccountStateToCloudIfSignedIn.
  ...
  /// 14.09.2026 eklendi (kanka isteği — "Siteyi Devret" özelliği) — [_stripImages]
  /// ile AYNI mantığı, bu dosya DIŞINDAN da (bkz. AppState.initiateProjectTransfer)
  /// kullanabilmek için açan ince bir sarmalayıcı. GEREKÇE: devir kodu üretilirken
  /// proje anlık görüntüsü Cloudflare Worker'ın D1 veritabanına (TEXT sütun)
  /// yazılıyor — buluta yazarken zaten uygulanan AYNI "gömülü base64 fotoğrafları
  /// çıkar" güvenlik payı burada da geçerli olmalı, aksi halde fotoğraf ağırlıklı
  /// bir proje (birkaç MB) D1'in pratik satır boyutunu aşıp devir isteği sessizce
  /// başarısız/bozuk kalabilirdi.
  ...
  dynamic _stripImages(dynamic value) {
    ...
    if (value is Map) {
      // 05.09.2026 eklendi — KRİTİK DÜZELTME (kanka'nın uygulama içi log
      // ekranından yakaladığı gerçek hata): `value.map((k, v) => ...)`
      // burada `value`'nun çalışma zamanı tipi (runtime type) sadece `Map`
      // (parametresiz) olduğu için, .map()'in ÇIKTI tipi de Dart tarafından
      // `Map<dynamic, dynamic>` olarak çıkarılıyordu — `Map<String, dynamic>`
      // DEĞİL. upsertProject'teki `as Map<String, dynamic>` bu yüzden HER
      // ÇAĞRIDA (istisnasız) `type '_Map<dynamic, dynamic>' is not a
      // subtype of type 'Map<String, dynamic>'` hatasıyla patlıyordu — yani
      // tek bir proje bile buluta hiç ulaşmamıştı, "network/rules" sorunu
      // DEĞİLMİŞ. `.map<String, dynamic>(...)` ile çıktı tipini elle
      // belirtip, anahtarı da `.toString()` ile garantiye alıyoruz.
