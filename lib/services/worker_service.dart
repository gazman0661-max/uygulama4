...
/// GEMINI_API_KEY'ini kullanan proxy). Uygulamadaki TÜM AI istekleri
/// buradan geçer — kullanıcı kendi API anahtarını girmez, aylık AI puan
/// kotasına tabidir (bkz. AppState.ensureAiQuotaFor / consumeAiQuota).
///
/// worker.js kendi prompt metinlerini sunucu tarafında kuruyor. Buradan
/// worker'a sadece HANGİ prompt'un hangi verilerle (previous_code,
/// current_code, section_html, current_background_css) kurulacağı
/// gönderiliyor; yanıt yorumlama (sentinel/reddetme kontrolü, diff
/// uygulama, CSS güvenlik ağı vb.) ai_response_utils.dart mantığıyla
/// istemci tarafında (burada) yapılıyor.
///
/// İSTEK: POST {
...
/// AI isteği sunucudan makul bir sürede yanıt gelmediğinde fırlatılır
/// (bkz. WorkerService._post > .timeout). Öncesinde bu durumda istek
/// SÜRESİZ beklerdi — zayıf bağlantıda kullanıcı "yükleniyor" ekranında
/// sonsuza kadar takılı kalabiliyordu, artık en geç 30 saniyede net bir
/// hataya dönüşüyor.
class WorkerTimeoutException implements Exception {
  final String message;
  WorkerTimeoutException(this.message);
  @override
  String toString() => message;
}
...
class WorkerService {
  ...
  /// Gemini'nin normal yanıt süresi (özellikle çok sayfalı site üretiminde)
  /// bazen 15-20 saniyeyi bulabiliyor; 30 saniye bu payı bırakırken yine de
  /// kullanıcıyı sonsuza kadar bekletmiyor.
  ...
  static Future<Map<String, dynamic>> _post(Map<String, dynamic> payload) async {
    ...
    try {
      ...
      throw WorkerTimeoutException(
          'Sunucudan yanıt alınamadı (zaman aşımı). Lütfen internet bağlantını kontrol edip tekrar dene.');
    } catch (e) {
