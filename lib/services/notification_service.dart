...
class NotificationService {
  ...
  // "Kutu" (mailbox) ve "Talepler" (lead) için ANLIK bildirimler — her
  // mesaj/talep kendi id'sini mailId/leadId.hashCode üzerinden bu tabanlara
  // ekleyerek alır, böylece aynı anda birden fazla yeni mesaj/talep gelirse
  // bildirimler birbirinin üstüne yazılmaz, ayrı ayrı bildirim tepsisinde
  // görünür (bkz. showNewMailboxMessageNotification / showNewLeadNotification).
  static const int _mailboxMessageBaseId = 8000000;
  static const int _leadMessageBaseId = 9000000;
  ...
  // ---------------------------------------------------------------------
  // 6) "Kutu" (mailbox) ve "Talepler" (lead) — ANLIK bildirimler.
  //
  // ÖNEMLİ SINIRLAMA (yukarıdaki günlük ziyaretçi hatırlatmasıyla AYNI
  // sebep): bu sınıf tamamen YEREL bildirim kullanıyor, gerçek bir push
  // (FCM) altyapısı YOK. Bu iki metod `zonedSchedule` değil `_plugin.show`
  // kullanır — yani "ileride bir tarihte göster" değil, ÇAĞRILDIĞI ANDA
  // gösterir. Bu yüzden sadece UYGULAMA AÇIKKEN (Firestore stream'i canlı
  // dinlenirken — bkz. main_shell.dart > _watchForNewMessages) tetiklenen
  // "yeni doküman geldi" olayına bağlı olarak çalışırlar. Uygulama TAMAMEN
  // KAPALIYKEN (arka planda süreç bile yokken) bir mesaj/talep gelirse bu
  // bildirim GÖSTERİLMEZ — bunun için gerçek bir push mimarisi (Cloudflare
  // Worker'da bir tetikleyici + FCM) ayrıca kurulması gerekir; bu, bu
  // sınıfın "sunucusuz/basit" tasarımının dışında, ayrı bir iştir (ilerideki
  // adım — bkz. proje notları).
  Future<void> showNewMailboxMessageNotification(String mailId) async {
    try {
      await init();
      final isEn = await _isEnglish();
      final title = isEn ? 'New message' : 'Yeni mesaj';
      final body = isEn
          ? 'You have a message in your inbox 📬'
          : 'Gelen kutunuzda mesajınız var 📬';
      final id = _mailboxMessageBaseId + (mailId.hashCode & 0x0FFFFFFF);
      await _plugin.show(id, title, body, _details, payload: mailId);
    } catch (e, st) {
      debugPrint(
          'showNewMailboxMessageNotification başarısız (yok sayılıyor): $e\n$st');
    }
  }

  Future<void> showNewLeadNotification(String leadId) async {
    try {
      await init();
      final isEn = await _isEnglish();
      final title = isEn ? 'New request' : 'Yeni talep';
      final body = isEn
          ? 'You have a message in your request inbox 📩'
          : 'Talep kutunuzda mesajınız var 📩';
      final id = _leadMessageBaseId + (leadId.hashCode & 0x0FFFFFFF);
      await _plugin.show(id, title, body, _details, payload: leadId);
    } catch (e, st) {
      debugPrint('showNewLeadNotification başarısız (yok sayılıyor): $e\n$st');
    }
  }
