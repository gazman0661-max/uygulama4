...
class NotificationService {
  ...
  // "1 Aylık Mini Paket" — DOMAIN YENİLEME HATIRLATMASIYLA AYNI DESEN
  // (bkz. _domainRenewalWarnBaseId/_domainRenewalExpiredBaseId), sadece
  // süre 1 yıl değil 1 ay olduğu için uyarı penceresi 30 gün değil 7
  // gündür (06.09.2026 eklendi, kanka isteği). Her proje kendi id'sini
  // projectId.hashCode üzerinden bu tabanlara ekleyerek alır (bkz.
  // scheduleMiniPackageExpiryReminder). 4000000 tabanlı showPushNotification
  // id aralığıyla (4000000 + 0x0FFFFFF'e kadar) çakışmaması için kasıtlı
  // olarak uzak tutuldu.
  ...
  // ---------------------------------------------------------------------
  // 1) ESKİ "aylık kota reset" bildirimi kaldırıldı (puan sistemi
  //    tamamen kaldırıldığı için, bkz. kanka isteği). Bu bildirim eskiden
  //    zonedSchedule ile TEKRARLANAN bir bildirimdi, yani yükseltme
  //    öncesinde uygulamayı açmış kullanıcıların cihazında hâlâ kurulu
  //    kalabilir — bu yüzden fonksiyonu silmek yetmiyor, eski id (1001)
  //    ile açıkça iptal etmek gerekiyor. Bunu her açılışta çağırmak
  //    zararsız (zaten kurulu değilse no-op).
  ...
  // ---------------------------------------------------------------------
  // 6) "1 Aylık Mini Paket" — SÜRE hatırlatması.
  //    Süre dolmadan 7 gün önce bir uyarı + bitiş gününde bir "süresi
  //    doldu" bildirimi. scheduleDomainRenewalReminder ile BİREBİR AYNI
  //    desen — tek fark 30 gün yerine 7 gün ve 1 yıl yerine 1 ay.
  // ---------------------------------------------------------------------
  ...
  /// AppState.activateMiniPackage içinden, hem gerçek satın alma hem de
  /// hediye koduyla redeemGiftMiniPackage sonrası (o da activateMiniPackage'i
  /// çağırıyor) TEK noktadan çağrılır. [expiresAt], SiteProject.
  /// miniPackageExpiresAt'ten gelir (aktivasyon anından TAM 30 gün sonrası).
  /// Önce eski planlanmış bildirimler iptal edilir (kullanıcı süre
  /// dolmadan tekrar satın alırsa/hediye kullanırsa süre sıfırdan 30 güne
  /// uzar — bkz. activateMiniPackage dokümantasyonu), sonra:
  ///   - bitişten 7 gün önce bir "yakında doluyor" uyarısı,
  ///   - bitiş gününün kendisinde bir "süresi doldu" bildirimi
  /// kurulur. Bu tarihlerden biri zaten geçmişse (ör. bir şekilde geçmiş
  /// bir tarihle çağrılırsa) o bildirim atlanır — geçmişe planlama
  /// yapılmaz (bkz. _noonOnDate).
  Future<void> scheduleMiniPackageExpiryReminder({
    ...
    try {
      ...
      // Bu metod activateMiniPackage İÇİNDEN çağrılıyor — asıl aktivasyon
      // akışını (rozet kaldırma, persist) asla etkilememeli.
  ...
  // ---------------------------------------------------------------------
  // 7) Günlük "sitenizi kim ziyaret etti" hatırlatması
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  // 8) "Kutu" (mailbox) ve "Talepler" (lead) — ANLIK bildirimler artık
